...
- [Caching](../caching/caching.md) -- timeline caches (Redis sorted sets) are the storage layer for fan-out on write
- [Redis](../caching/redis.md) -- sorted sets for timeline storage, Pub/Sub for real-time fan-out
- [Message Queues](../messaging/message-queues.md) -- Kafka/SQS decouple post creation from asynchronous fan-out processing
- [Real-Time Protocols](../api-design/real-time-protocols.md) -- SSE/WebSockets deliver fan-out results to connected clients
- [Sharding](../scalability/sharding.md) -- celebrity hot keys require sharding strategies (random suffixes, dedicated shards)
