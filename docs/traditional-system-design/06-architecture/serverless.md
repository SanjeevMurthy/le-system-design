...
- [Event-Driven Architecture](../05-messaging/event-driven-architecture.md) -- Lambda is inherently event-driven; EventBridge is the serverless event bus
- [Autoscaling](../02-scalability/autoscaling.md) -- serverless is the ultimate auto-scaling: scale to zero, scale to thousands
- [Rate Limiting](../08-resilience/rate-limiting.md) -- API Gateway rate limiting protects Lambda from overload
- [DynamoDB](../03-storage/dynamodb.md) -- the canonical serverless database; Streams enable CDC triggers
- [Message Queues](../05-messaging/message-queues.md) -- SQS as a Lambda trigger for asynchronous processing
- [CQRS](../05-messaging/cqrs.md) -- serverless naturally supports CQRS with separate read/write Lambda functions
- [Circuit Breaker](../08-resilience/circuit-breaker.md) -- Step Functions implement circuit breaker patterns for external API calls
- [Distributed Transactions](../08-resilience/distributed-transactions.md) -- Step Functions implement the saga pattern for multi-step workflows
