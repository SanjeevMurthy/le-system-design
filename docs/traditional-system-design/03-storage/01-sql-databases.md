...
- **Index**: A secondary data structure (typically B-tree) that accelerates lookups at the cost of write overhead. See [Database Indexing](./04-database-indexing.md).
...
This is why storing large binary objects (4 MB images) in Postgres is catastrophic --- a single image spans 500 pages, bloating the buffer pool, the TOAST table, and replication throughput. Always store media in [Object Storage](./03-object-storage.md) and keep only the URL in the database.
...
2. **Read replicas**: Route read queries to follower replicas, keeping the leader for writes only. See [Database Replication](./05-database-replication.md).
...
5. **Caching**: Place a Redis or Memcached layer in front of the database for frequently accessed, read-heavy data. See [Caching Strategies](../04-caching/01-caching.md).
...
7. **Sharding (last resort)**: Distribute data across multiple database instances by a shard key. This breaks joins, foreign keys, and cross-shard transactions. See [Sharding](../02-scalability/04-sharding.md).
...
- **Storing BLOBs in the database**: A 4 MB image in Postgres spans 500 pages, bloats replication, and destroys backup performance. Store media in [Object Storage](./03-object-storage.md) and keep only the URL in the database.
- **Missing indexes on query columns**: A full table scan on 100M rows takes seconds. Always index columns used in WHERE, JOIN, and ORDER BY clauses. See [Database Indexing](./04-database-indexing.md).
...
- **Premature sharding**: Sharding a relational database breaks joins, foreign keys, and transactions. Exhaust vertical scaling, read replicas, and caching before you shard. See [Sharding](../02-scalability/04-sharding.md).
...
- [NoSQL Databases](./02-nosql-databases.md) --- when to choose non-relational alternatives
- [Database Indexing](./04-database-indexing.md) --- B-trees, LSM trees, and the write penalty
- [Database Replication](./05-database-replication.md) --- leader/follower, WAL shipping, CDC
- [Sharding](../02-scalability/04-sharding.md) --- horizontal partitioning when vertical scaling is exhausted
- [Distributed Transactions](../08-resilience/03-distributed-transactions.md) --- 2PC and saga patterns for cross-service ACID
