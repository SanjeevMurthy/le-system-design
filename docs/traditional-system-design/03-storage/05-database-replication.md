...
Gossip avoids the single-point-of-failure of centralized coordinators like ZooKeeper while converging quickly even in large clusters. See [CAP Theorem](../01-fundamentals/05-cap-theorem.md) for how gossip relates to consistency models.
...
- [SQL Databases](./01-sql-databases.md) --- WAL and ACID transactions depend on replication for durability
- [Cassandra](./07-cassandra.md) --- leaderless replication with tunable consistency
- [DynamoDB](./08-dynamodb.md) --- DynamoDB Streams as a CDC mechanism
- [CAP Theorem](../01-fundamentals/05-cap-theorem.md) --- consistency models and partition tolerance
- [Consistent Hashing](../02-scalability/03-consistent-hashing.md) --- data placement in leaderless replication
