...
This file covers caching **strategies, layers, and pitfalls** as general architectural patterns. For Redis-specific internals (data structures, commands, clustering), see [Redis](./02-redis.md). For CDN edge caching, see [CDN](./03-cdn.md).
...
- [Redis](./02-redis.md) --- Redis-specific data structures, commands, and hot key solutions
- [CDN](./03-cdn.md) --- edge caching layer for static assets
- [Database Replication](../03-storage/05-database-replication.md) --- read replicas as an alternative to caching for read scaling
- [Rate Limiting](../08-resilience/01-rate-limiting.md) --- rate limiters often implemented on top of cache infrastructure
- [Consistent Hashing](../02-scalability/03-consistent-hashing.md) --- distributing cache keys across a cluster
...
- source/extracted/grokking/ch81-01-caching.md (Cache invalidation)
