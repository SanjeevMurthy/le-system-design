...
- [Caching](../04-caching/01-caching.md) -- timeline caches (Redis sorted sets) are the storage layer for fan-out on write
- [Redis](../04-caching/02-redis.md) -- sorted sets for timeline storage, Pub/Sub for real-time fan-out
- [Message Queues](../05-messaging/01-message-queues.md) -- Kafka/SQS decouple post creation from asynchronous fan-out processing
- [Real-Time Protocols](../07-api-design/04-real-time-protocols.md) -- SSE/WebSockets deliver fan-out results to connected clients
- [Sharding](../02-scalability/04-sharding.md) -- celebrity hot keys require sharding strategies (random suffixes, dedicated shards)
