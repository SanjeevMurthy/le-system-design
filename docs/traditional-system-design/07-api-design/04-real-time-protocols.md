...
- [Fan-Out](../11-patterns/01-fan-out.md) -- real-time protocols are the delivery mechanism for fan-out on write
- [Message Queues](../05-messaging/01-message-queues.md) -- Kafka/Redis Pub/Sub form the backend for SSE/WS event distribution
- [Redis](../04-caching/02-redis.md) -- Pub/Sub and connection registry for real-time servers
- [GraphQL](03-graphql.md) -- subscriptions use WebSockets for real-time updates
- [Video Streaming](../11-patterns/04-video-streaming.md) -- WebRTC for live, HLS/DASH for on-demand
