...
- [CQRS](./04-cqrs.md) -- frequently combined with event sourcing; events propagate changes to read models
- [Event-Driven Architecture](./02-event-driven-architecture.md) -- event sourcing is a specific implementation of EDA principles
- [Message Queues](./01-message-queues.md) -- Kafka/Kinesis as the event store infrastructure
- [Distributed Transactions](../08-resilience/03-distributed-transactions.md) -- saga pattern uses event logs for coordination
- [Database Replication](../03-storage/05-database-replication.md) -- CDC as a related but distinct pattern
...
- source/extracted/acing-system-design/ch07-03-distributed-transactions.md (event sourcing definition, event log as source of truth, CDC comparison, audit trail)
