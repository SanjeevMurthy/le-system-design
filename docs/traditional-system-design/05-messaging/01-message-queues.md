...
- [Event-Driven Architecture](./02-event-driven-architecture.md) -- pub/sub patterns, choreography, and stream processing built on message queues
- [Event Sourcing](./03-event-sourcing.md) -- Kafka as an append-only event store
- [CQRS](./04-cqrs.md) -- using message queues to propagate commands to read models
- [Rate Limiting](../08-resilience/01-rate-limiting.md) -- backpressure and throttling at the producer level
- [Redis](../04-caching/02-redis.md) -- Redis Streams as a lightweight alternative for simple queuing
...
- source/extracted/acing-system-design/ch07-03-distributed-transactions.md (Kafka in saga patterns)
