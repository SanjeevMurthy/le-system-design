...
2. **API Gateway rate limiting**: Enforces per-client request rate limits. See [Rate Limiting](../08-resilience/01-rate-limiting.md) for algorithms (token bucket, sliding window). Rate limits should be keyed to authenticated identity, not just IP, since attackers rotate IPs.
...
5. **Virtual Waiting Room**: For predictable surge events (ticket sales, product launches), a queue-based admission system admits users at a controlled rate. See [Rate Limiting](../08-resilience/01-rate-limiting.md) for details.
...
- [Authentication and Authorization](01-authentication-authorization.md) -- AuthN/AuthZ is the identity foundation that API security builds upon
- [Encryption](02-encryption.md) -- TLS encryption is the transport security layer for all API communication
- [Rate Limiting](../08-resilience/01-rate-limiting.md) -- algorithms and patterns for request throttling (cross-link only; rate limiting details live there)
- [API Gateway](../06-architecture/01-api-gateway.md) -- the centralized enforcement point for API security policies
- [REST API](../07-api-design/01-rest-api.md) -- secure design patterns for RESTful APIs
- [Circuit Breaker](../08-resilience/02-circuit-breaker.md) -- protects services from cascading failures when security-related checks timeout
