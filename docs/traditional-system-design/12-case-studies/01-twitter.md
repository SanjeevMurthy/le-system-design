...
Twitter does not use a single fan-out model. It employs a [hybrid fan-out](../11-patterns/01-fan-out.md) approach:
...
Tweets are JSON-like documents with variable metadata (hashtags, media references, retweet flags). A document store (Twitter uses Manhattan internally; MongoDB is a functional equivalent for design discussions) is ideal because it avoids the rigid schema overhead of SQL while supporting fast primary-key lookups. See [NoSQL databases](../03-storage/02-nosql-databases.md).

### CDC-Powered Search Indexing
Full-text search on tweets uses an [inverted index via Elasticsearch](../11-patterns/05-search-and-indexing.md). Rather than having the tweet service write to both the primary store and the search index (which couples them), Twitter uses [CDC (Change Data Capture)](../03-storage/05-database-replication.md) to stream mutations from the tweet store to Elasticsearch asynchronously. This keeps the write path clean and the search index eventually consistent.

### Timeline Cache in Redis
Pre-computed timelines are stored in [Redis](../04-caching/02-redis.md) sorted sets, keyed by user ID with tweet IDs as members and timestamps as scores. This allows efficient range queries (fetch the latest 200 tweets) and O(log N) insertions from the fan-out service.
...
Tweet search uses a [full-text inverted index](../11-patterns/05-search-and-indexing.md) built on Elasticsearch:
...
- **Hot terms**: A [Count-Min Sketch](../11-patterns/02-probabilistic-data-structures.md) identifies trending search terms, allowing the system to pre-warm caches for popular queries.
...
4. Media attachments are uploaded via [pre-signed URLs](../03-storage/03-object-storage.md) directly to S3, bypassing the application server.
...
**Rate limits**: Twitter enforces strict [rate limiting](../08-resilience/01-rate-limiting.md) -- 300 tweet creates per 3 hours, 900 timeline reads per 15-minute window. These limits protect the fan-out service and cache from abuse while remaining transparent to normal user behavior.
...
The fan-out service is the most write-intensive component. It is horizontally scaled via [Kafka consumer groups](../05-messaging/01-message-queues.md), with each consumer handling a partition of tweet events. Hot partitions caused by celebrity tweets are mitigated by the hybrid approach -- celebrities are simply skipped. The consumer group auto-rebalances when new consumers are added or existing ones fail.
...
Redis is [clustered](../04-caching/02-redis.md) across 16,384 slots using [consistent hashing](../02-scalability/03-consistent-hashing.md). Hot keys (e.g., a viral user's timeline receiving millions of reads) are mitigated using the N-cache strategy: instead of sharding the cache (which keeps the hot key on one node), the system replicates the hot key across multiple cache instances. The timeline service randomly selects one of N replicas for each read, distributing load evenly.

### Read Scaling
Timeline reads are served entirely from cache in the common case. Cache misses fall through to the tweet store, which uses [sharding](../02-scalability/04-sharding.md) by tweet ID for even distribution. The cache-aside pattern ensures that missed keys are populated on first access and served from cache on subsequent requests.
...
### CDN for Media
All images and videos are served via [CDN edge locations](../04-caching/03-cdn.md), keeping media reads off the origin servers entirely. For a platform generating 200M+ media uploads per day, the CDN absorbs 99%+ of media read traffic, with only cold or newly uploaded content hitting the S3 origin.
...
- **Hot tweets**: A viral tweet may receive millions of likes per hour. The engagement counter key becomes a hot key. Mitigation: use the [random suffix strategy](../04-caching/02-redis.md) (`tweet_engagement:{tweet_id}:{shard}`) with a final summation across shards at read time.
...
| Redis timeline cache node dies | Timeline reads for affected users fail | Redis cluster redistributes slots; [cache-aside](../04-caching/01-caching.md) pattern falls through to DB; cache warming rebuilds hot timelines |
...
| Celebrity tweet goes viral | Massive surge in read-time merges | [Rate limiting](../08-resilience/01-rate-limiting.md) on timeline API; caching of celebrity tweet lists with short TTL (30s) |
...
**Mitigation chain**:
- [Circuit breaker](../08-resilience/02-circuit-breaker.md) on the tweet store trips after error rate exceeds threshold, failing fast and returning cached (possibly stale) timelines.
...
- [Fan-out (read vs. write, hybrid)](../11-patterns/01-fan-out.md)
- [Redis (sorted sets, pub/sub, clustering, hot key mitigation)](../04-caching/02-redis.md)
- [Search and indexing (inverted index, Elasticsearch, CDC sync)](../11-patterns/05-search-and-indexing.md)
- [Probabilistic data structures (Count-Min Sketch for trending)](../11-patterns/02-probabilistic-data-structures.md)
- [NoSQL databases (document stores, Manhattan)](../03-storage/02-nosql-databases.md)
- [Object storage (pre-signed URLs for media)](../03-storage/03-object-storage.md)
- [CDN (edge caching for media)](../04-caching/03-cdn.md)
- [Sharding (hash-based partitioning)](../02-scalability/04-sharding.md)
- [Message queues (Kafka for event streaming)](../05-messaging/01-message-queues.md)
- [Rate limiting (protecting against viral surges)](../08-resilience/01-rate-limiting.md)
- [Caching strategies (cache-aside, TTL)](../04-caching/01-caching.md)
