...
Netflix splits its architecture into two distinct planes. The **control plane** -- all business logic services (user management, recommendations, search, billing, playback session management) -- runs on AWS across multiple regions. The **data plane** -- the actual delivery of video bytes to viewers -- runs on Netflix's proprietary Open Connect CDN. This separation means that even if AWS experiences degradation, cached content on OCAs continues streaming. The control plane uses [microservices architecture](../06-architecture/microservices.md) with hundreds of independent services.

### Microservices with Zuul API Gateway
All client requests route through Zuul, Netflix's custom [API gateway](../06-architecture/api-gateway.md), which handles authentication, routing, rate limiting, canary testing, and request context propagation. Behind Zuul, 1,000+ microservices communicate via a mix of [gRPC](../07-api-design/grpc.md) for synchronous calls and [Kafka](../05-messaging/message-queues.md) for asynchronous event-driven workflows. [Service discovery](../06-architecture/microservices.md) is handled by Eureka, Netflix's homegrown service registry.

### Transcoding as a Massively Parallel Pipeline
Studio masters are ingested and split into short segments. Each segment is independently encoded across a fleet of thousands of EC2 instances, enabling massive parallelism. Encoding produces variants across multiple dimensions (resolution, bitrate, codec, audio, subtitles), resulting in thousands of files per title. The pipeline uses a [message queue](../05-messaging/message-queues.md) (SQS/Kafka) for job distribution and is designed for retry and idempotency -- if a worker fails mid-encode, the segment is re-queued and processed by another worker.
...
**Fault tolerance**: Each encoding job is idempotent. If a worker crashes mid-encode, the job returns to the queue after a visibility timeout. The scheduler tracks job completion and only assembles the manifest after all segments for all variants are complete. A [dead-letter queue](../05-messaging/message-queues.md) captures persistently failing jobs for manual investigation.
...
Netflix uses [HTTP-based adaptive streaming protocols](../11-patterns/video-streaming.md) (primarily DASH with HLS fallback for Apple devices). Video content is segmented into small chunks (typically 2-4 seconds each), and each chunk is available in multiple quality levels (the bitrate ladder).
...
**Architecture follows the [three-stage recommendation pipeline](../11-patterns/recommendation-engines.md):**
...
- *Content-based filtering*: Content features (genre, director, cast, mood tags, visual style) are encoded as [vector embeddings](../11-patterns/recommendation-engines.md). Content similar to what the user has watched is retrieved via approximate nearest neighbor (ANN) search.
...
The ranking model is trained on billions of user-content interaction events using distributed [batch processing (Spark)](../05-messaging/event-driven-architecture.md) and updated periodically.
...
**Training infrastructure**: The recommendation models are trained on Netflix's data platform, which processes petabytes of user behavior data. [Event-driven pipelines](../05-messaging/event-driven-architecture.md) capture every user interaction (play, pause, seek, browse, search, rate) as structured events. These events flow through [stream processing (Flink/Mantis)](../05-messaging/event-driven-architecture.md) for real-time feature computation and are landed in a data lake for batch model training.

**Serving latency**: Pre-computed recommendations are stored in EVCache ([distributed caching](../04-caching/caching.md)) and refreshed periodically (every few hours or on significant user activity). When a user opens the app, the recommendation service reads from cache, applies lightweight reranking (accounting for time-of-day and recently watched), and returns the personalized homepage in < 200ms.
...
The transcoding pipeline is the most compute-intensive component. It scales horizontally by launching more EC2 instances (often Spot Instances for cost efficiency) when the encoding queue depth increases. Netflix processes thousands of encoding jobs concurrently. [Autoscaling](../02-scalability/autoscaling.md) policies monitor SQS/Kafka queue depth and adjust the worker fleet. Spot Instance interruptions are handled gracefully -- the interrupted job returns to the queue and is picked up by another worker. This pattern leverages [message queue guarantees](../05-messaging/message-queues.md) (at-least-once delivery with idempotent processing) for resilience.

### Microservices Scaling
Each of Netflix's 1,000+ microservices is independently deployable and scalable. Services are containerized and orchestrated via Titus (Netflix's container management platform, built on top of AWS). [Load balancing](../02-scalability/load-balancing.md) (Ribbon, a client-side load balancer) distributes requests across service instances. Critical services (playback, recommendations, user auth) are scaled to handle 10x their average traffic to absorb spikes.
...
### Database Scaling
- **PostgreSQL** (users, billing, catalog): [Sharded](../02-scalability/sharding.md) by user_id for user tables; read replicas handle catalog queries. [Database replication](../03-storage/database-replication.md) provides HA with automatic leader failover.
...
- **EVCache**: Scaled by adding nodes to the cache cluster. [Consistent hashing](../02-scalability/consistent-hashing.md) distributes keys across nodes. Netflix runs EVCache in multiple availability zones with replication for fault tolerance.
...
- **Pre-warming**: Infrastructure is scaled up before predicted peaks. [Feature flags](../08-resilience/feature-flags.md) disable non-critical features (recommendation model retraining, analytics batch jobs) during peak to free compute resources.
...
| Recommendation service timeout | Homepage shows stale or generic recommendations | EVCache serves stale cached recommendations; generic "popular in your country" row is displayed as fallback; [circuit breaker](../08-resilience/circuit-breaker.md) prevents cascading failure |
...
| DRM license service failure | New playback sessions cannot start; existing sessions continue | DRM licenses have TTL (hours); active viewers are unaffected; [circuit breaker](../08-resilience/circuit-breaker.md) on license service fails fast; service is multi-region for HA |
| EVCache cluster failure | Read latency spikes as requests fall through to database | [Cache-aside](../04-caching/caching.md) pattern; database absorbs temporary load spike; cache rebuilds within minutes; services implement request coalescing to prevent [cache stampede](../04-caching/caching.md) |
...
This proactive approach to resilience testing is built on [circuit breaker patterns](../08-resilience/circuit-breaker.md) and [feature flags](../08-resilience/feature-flags.md) that enable rapid kill-switch activation for degraded components.
...
- [Video streaming (ABR, HLS, MPEG-DASH, transcoding, chunked delivery)](../11-patterns/video-streaming.md) -- core delivery mechanism
- [CDN (edge caching, content delivery, push vs. pull)](../04-caching/cdn.md) -- Open Connect architecture
- [Recommendation engines (three-stage pipeline, collaborative filtering, embeddings)](../11-patterns/recommendation-engines.md) -- personalization
- [Message queues (Kafka, SQS, consumer groups, DLQ, back-pressure)](../05-messaging/message-queues.md) -- encoding pipeline and event streaming
- [Event-driven architecture (pub/sub, stream processing, Flink)](../05-messaging/event-driven-architecture.md) -- analytics and ML pipelines
- [Microservices (service discovery, bounded contexts, independent deployment)](../06-architecture/microservices.md) -- service architecture
- [API gateway (Zuul, routing, rate limiting, auth)](../06-architecture/api-gateway.md) -- entry point
- [Circuit breaker (Hystrix, cascading failure prevention)](../08-resilience/circuit-breaker.md) -- resilience
- [Feature flags (canary releases, kill switches)](../08-resilience/feature-flags.md) -- operational control and chaos engineering
- [Caching strategies (cache-aside, EVCache, cache stampede prevention)](../04-caching/caching.md) -- data access optimization
- [Object storage (S3, mezzanine and encoded asset storage)](../03-storage/object-storage.md) -- media storage
- [SQL databases (PostgreSQL for users, billing, catalog)](../03-storage/sql-databases.md) -- structured data
- [Cassandra (watch history, content metadata, tunable consistency)](../03-storage/cassandra.md) -- high-write data
- [NoSQL databases (polyglot persistence)](../03-storage/nosql-databases.md) -- storage strategy
- [Search and indexing (Elasticsearch, CDC-based sync)](../11-patterns/search-and-indexing.md) -- content search
- [Sharding (hash-based partitioning)](../02-scalability/sharding.md) -- database scaling
- [Consistent hashing (EVCache distribution)](../02-scalability/consistent-hashing.md) -- cache scaling
- [Autoscaling (transcoding workers, microservices)](../02-scalability/autoscaling.md) -- elastic capacity
- [Load balancing (Ribbon, client-side LB)](../02-scalability/load-balancing.md) -- traffic distribution
- [Rate limiting (API protection)](../08-resilience/rate-limiting.md) -- abuse prevention
- [Distributed transactions (saga pattern for encoding pipeline)](../08-resilience/distributed-transactions.md) -- pipeline orchestration
- [Database replication (PostgreSQL HA, Cassandra leaderless)](../03-storage/database-replication.md) -- data durability
- [Back-of-envelope estimation](../01-fundamentals/back-of-envelope-estimation.md) -- capacity planning
- [Availability and reliability](../01-fundamentals/availability-reliability.md) -- SLA targets
- [Encryption (DRM, TLS)](../09-security/encryption.md) -- content protection
- [Monitoring (Prometheus, Grafana, Atlas)](../10-observability/monitoring.md) -- system health
- [Logging (centralized logging, distributed tracing)](../10-observability/logging.md) -- troubleshooting
