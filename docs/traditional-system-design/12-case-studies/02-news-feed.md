...
The [hybrid fan-out](../11-patterns/01-fan-out.md) strategy is governed by a `pre_computed` flag on each user/page in the social graph:
...
For features like "trending topics" or search typeahead on the feed, the system uses a [Count-Min Sketch](../11-patterns/02-probabilistic-data-structures.md) to efficiently track the frequency of terms, hashtags, and URLs across all posts. The Count-Min Sketch:
...
Hot terms detected by the Count-Min Sketch are used to pre-warm the [search index](../11-patterns/05-search-and-indexing.md) cache and power trending topic modules.

### Social Graph in SQL
The social graph (who follows whom) is stored in [PostgreSQL](../03-storage/01-sql-databases.md). Friend relationships are bidirectional (unlike Twitter's unidirectional follows), so the graph is modeled as an undirected edge list. SQL is chosen because:
...
The ranking model is a simplified version of a [recommendation engine](../11-patterns/03-recommendation-engines.md):
...
1. **Stream processing**: As posts are created, a [Flink](../05-messaging/02-event-driven-architecture.md) job extracts hashtags, mentioned entities, and URLs.
...
- **Fan-out service**: The highest write-volume component. Horizontally scaled via [Kafka consumer groups](../05-messaging/01-message-queues.md), with each consumer handling a partition of post events. For a post by a user with 1,000 followers, the fan-out generates 1,000 Redis writes -- handled by multiple consumers in parallel.
- **Feed cache**: [Redis cluster](../04-caching/02-redis.md) with 16,384 slots. Each user's feed is a sorted set capped at 500 entries (~4KB per user). For 500M DAU: ~2TB of feed cache. Hot users (those with many followers posting) create hot keys -- mitigated by random suffix sharding.
- **Post store**: [Sharded](../02-scalability/04-sharding.md) by post_id for even distribution. Celebrity posts are read frequently but are easily cacheable with short TTLs.
- **Social graph**: Read-heavy and relatively static. [Database replication](../03-storage/05-database-replication.md) with multiple read replicas handles the query volume. The graph is sharded by user_id.
...
| Feed cache (Redis) node failure | Feed reads fail for affected users | Redis cluster redistributes slots; [cache-aside](../04-caching/01-caching.md) fallback queries post store + social graph to reconstruct feed |
...
- [Fan-out (read vs. write, hybrid, pre_computed flag)](../11-patterns/01-fan-out.md)
- [Redis (sorted sets for feed cache, affinity cache, trending cache)](../04-caching/02-redis.md)
- [Caching strategies (cache-aside fallback, TTL management)](../04-caching/01-caching.md)
- [Probabilistic data structures (Count-Min Sketch for trending)](../11-patterns/02-probabilistic-data-structures.md)
- [Search and indexing (inverted index for post search, CDC sync)](../11-patterns/05-search-and-indexing.md)
- [Recommendation engines (ranking model, affinity scoring)](../11-patterns/03-recommendation-engines.md)
- [Message queues (Kafka for post event streaming)](../05-messaging/01-message-queues.md)
- [SQL databases (social graph, ACID for friend relationships)](../03-storage/01-sql-databases.md)
- [Sharding (post store by post_id, social graph by user_id)](../02-scalability/04-sharding.md)
- [Database replication (read replicas for social graph)](../03-storage/05-database-replication.md)
- [NoSQL databases (post store for flexible document structure)](../03-storage/02-nosql-databases.md)
- [CDN (media delivery for post images/videos)](../04-caching/03-cdn.md)
