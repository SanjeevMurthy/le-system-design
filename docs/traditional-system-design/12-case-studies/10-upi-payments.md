...
- **Security**: End-to-end [encryption](../09-security/02-encryption.md) in transit (TLS 1.3); multi-factor [authentication](../09-security/01-authentication-authorization.md) (device binding, UPI PIN).
...
This layered model is analogous to the [API gateway](../06-architecture/01-api-gateway.md) pattern at a national scale -- the partner bank acts as a gateway that validates, authenticates, and routes requests into the secure NPCI network.
...
UPI's transaction integrity is achieved without a traditional [two-phase commit](../08-resilience/03-distributed-transactions.md) (which would require all parties to be synchronously available). Instead, it uses a **saga-like orchestrated pattern**:
...
UPI employs defense-in-depth [authentication](../09-security/01-authentication-authorization.md):
...
4. **Encryption in transit**: All communication between PSP, bank, and NPCI uses [TLS 1.3](../09-security/02-encryption.md). Additionally, the NPCI network is a private, air-gapped network -- it is not accessible from the public internet.
...
### PSP Scaling
PSP app servers are standard [horizontally scaled](../01-fundamentals/03-scaling-overview.md) stateless services behind [load balancers](../02-scalability/01-load-balancing.md). Google Pay, for example, handles hundreds of millions of users with a conventional [microservices](../06-architecture/02-microservices.md) architecture. The PSP's partner bank is the actual bottleneck, not the PSP app itself.
...
- The closed-loop ecosystem (PSP -> Partner Bank -> NPCI -> Receiver Bank) is a regulatory-driven architectural pattern that separates innovation (PSP layer) from trust (banking layer). This is analogous to the [API gateway](../06-architecture/01-api-gateway.md) pattern at national infrastructure scale.
...
- Rollback on ACK failure implements a [saga pattern](../08-resilience/03-distributed-transactions.md) where NPCI is the orchestrator. This avoids the impracticality of distributed two-phase commits at 100K+ TPS.
...
- [Distributed transactions (saga pattern, compensating transactions, 2PC comparison)](../08-resilience/03-distributed-transactions.md)
- [Encryption (TLS 1.3, encryption at rest and in transit)](../09-security/02-encryption.md)
- [Authentication and authorization (multi-factor auth, device binding)](../09-security/01-authentication-authorization.md)
- [API gateway (partner bank as a regulatory gateway)](../06-architecture/01-api-gateway.md)
- [Rate limiting (NPCI throttling per PSP/bank)](../08-resilience/01-rate-limiting.md)
- [Circuit breaker (handling bank outages gracefully)](../08-resilience/02-circuit-breaker.md)
- [Load balancing (PSP horizontal scaling)](../02-scalability/01-load-balancing.md)
- [Scaling overview (horizontal scaling of stateless PSP services)](../01-fundamentals/03-scaling-overview.md)
