...
2. **Demand measurement**: Count ride requests per H3 cell over a rolling window (e.g., last 5 minutes). Request events flow through Kafka into a [stream processing](../05-messaging/event-driven-architecture.md) engine (Apache Flink) that maintains windowed counters per cell.
...
- Solution: [Shard](../02-scalability/sharding.md) by `driver_id` across 10-20 Redis instances using [consistent hashing](../02-scalability/consistent-hashing.md).
...
1. **Road graph representation**: The world's road network is represented as a directed weighted graph stored in routing tiles (similar to the approach described in the Google Maps case study). Routing tiles are stored in [object storage (S3)](../03-storage/object-storage.md) and cached aggressively on routing service instances.
...
[Cassandra](../03-storage/cassandra.md) is chosen for its high write throughput and time-series-friendly data model. With 1.5M writes/sec and data retention of 30-90 days, this table grows rapidly and benefits from Cassandra's compaction and TTL-based expiration.
...
At 1.5M location updates/sec, the location service is horizontally scaled behind a [load balancer](../02-scalability/load-balancing.md). Each instance handles a shard of driver IDs and writes to the corresponding Redis shard. Kafka producers are batched (linger.ms=5, batch.size=16KB) to reduce network overhead.
...
Payment is a critical path requiring [strong consistency](../01-fundamentals/cap-theorem.md). The payment service uses a two-phase approach:
...
3. **Settlement**: Driver payouts are batched weekly, aggregated across all completed trips, using a [saga pattern](../08-resilience/distributed-transactions.md) to coordinate between the trip store, payment gateway, and driver wallet.
...
| Payment gateway timeout | Rider charged but driver not paid (or vice versa) | Idempotent retry with gateway transaction IDs; [saga pattern](../08-resilience/distributed-transactions.md) compensating transactions; manual reconciliation queue for unresolved cases |
...
| Notification service overload | Riders and drivers do not receive push updates | [Circuit breaker](../08-resilience/circuit-breaker.md) on notification service; fall back to SMS for critical notifications (ride assigned, trip completed) |
...
5. **Mitigation chain**:
   - [Autoscaling](../02-scalability/autoscaling.md) triggers additional matching service instances (pre-scaled based on historical NYE patterns).
   - [Rate limiting](../08-resilience/rate-limiting.md) at the API gateway throttles ride requests beyond capacity, queuing excess requests.
...
- H3 hexagonal cells provide uniform adjacency for geospatial operations, avoiding the corner-adjacency and boundary problems of rectangular geohashes. See [geospatial indexing](../11-patterns/geospatial-indexing.md) for the full comparison of indexing approaches.
...
- [Geospatial Indexing (H3, S2, geohash, quadtree)](../11-patterns/geospatial-indexing.md)
- [Redis (sorted sets, TTL, geospatial API)](../04-caching/redis.md)
- [Message Queues (Kafka for location streaming)](../05-messaging/message-queues.md)
- [Event-Driven Architecture (stream processing with Flink)](../05-messaging/event-driven-architecture.md)
- [Consistent Hashing (Redis sharding)](../02-scalability/consistent-hashing.md)
- [Sharding (location cache, Cassandra)](../02-scalability/sharding.md)
- [PostgreSQL / SQL Databases (trip store, payments)](../03-storage/sql-databases.md)
- [Cassandra (location history)](../03-storage/cassandra.md)
- [Object Storage (routing tiles)](../03-storage/object-storage.md)
- [Real-Time Protocols (WebSocket for trip tracking)](../07-api-design/real-time-protocols.md)
- [Distributed Transactions / Saga Pattern (payments)](../08-resilience/distributed-transactions.md)
- [Rate Limiting (API gateway throttling)](../08-resilience/rate-limiting.md)
- [Circuit Breaker (notification service fallback)](../08-resilience/circuit-breaker.md)
- [Autoscaling (pre-scaling for demand spikes)](../02-scalability/autoscaling.md)
- [Load Balancing (location service, matching service)](../02-scalability/load-balancing.md)
- [CAP Theorem (eventual consistency for locations, strong for trips)](../01-fundamentals/cap-theorem.md)
- [Back-of-Envelope Estimation](../01-fundamentals/back-of-envelope-estimation.md)
