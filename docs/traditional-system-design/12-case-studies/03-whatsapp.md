...
- **Security**: End-to-end [encryption](../09-security/02-encryption.md) -- the server never sees plaintext message content.
...
Unlike live comments (unidirectional, SSE), messaging requires the client to both send and receive messages over the same persistent connection. [WebSockets](../07-api-design/04-real-time-protocols.md) provide full-duplex communication over a single TCP connection. Each user's device maintains one WebSocket connection to a connection server.

### Redis Pub/Sub for Message Routing
When User A sends a message to User B, the message service must route it to the specific connection server holding User B's WebSocket. [Redis pub/sub](../04-caching/02-redis.md) provides this routing layer:
...
The system uses [ZooKeeper (service discovery)](../06-architecture/02-microservices.md) to maintain a mapping of `deviceID -> connectionServerHost`. When a device connects, the connection server registers the mapping. When the message service needs to route a message, it looks up the recipient's host via ZooKeeper.
...
**Implementation:**
- Each connection server periodically reports its connected users' status to a shared [Redis](../04-caching/02-redis.md) instance.
...
**Scaling consideration**: Presence updates are high-frequency (one heartbeat per connection per 10-30 seconds). For 100M concurrent users, this is 3-10M writes/sec to Redis. The presence store is sharded across multiple Redis nodes using [consistent hashing](../02-scalability/03-consistent-hashing.md).
...
WhatsApp uses the Signal Protocol for [end-to-end encryption](../09-security/02-encryption.md):
...
### Connection Server Fleet
Each server handles ~200K concurrent WebSocket connections. For 500M concurrent users at peak, the fleet needs ~2,500 connection servers. Connection servers are stateful (they hold WebSocket sessions), making them harder to scale than stateless services. [ZooKeeper](../06-architecture/02-microservices.md) manages host assignment, and ephemeral nodes provide automatic cleanup when a server fails.
...
Groups with 1,000 members create 1,000 inbox writes and 1,000 pub/sub publishes. This is handled asynchronously via [Kafka](../05-messaging/01-message-queues.md) to avoid blocking the sender's response. The sender receives a 200 OK as soon as the message is persisted to Kafka. Consumer workers then perform the fan-out to each member's inbox and pub/sub channel.
...
- [Real-time protocols (WebSockets for bidirectional communication)](../07-api-design/04-real-time-protocols.md)
- [Redis (pub/sub for message routing, presence with TTL)](../04-caching/02-redis.md)
- [Message queues (Kafka for async group fan-out)](../05-messaging/01-message-queues.md)
- [Cassandra (inbox store, high write throughput)](../03-storage/07-cassandra.md)
- [Microservices (ZooKeeper for service discovery / connection mapping)](../06-architecture/02-microservices.md)
- [Encryption (end-to-end encryption for message privacy)](../09-security/02-encryption.md)
- [Consistent hashing (Redis cluster sharding for presence)](../02-scalability/03-consistent-hashing.md)
- [CAP theorem (eventual consistency for presence, at-least-once for messages)](../01-fundamentals/05-cap-theorem.md)
- [Circuit breaker (handling downstream failures gracefully)](../08-resilience/02-circuit-breaker.md)
