...
The system uses a [Kappa architecture](../05-messaging/event-driven-architecture.md) where both real-time processing and historical reprocessing flow through the same stream processing engine (Flink). This avoids the dual-codebase problem of Lambda architecture, where separate stream and batch paths must be maintained and kept in sync.
...
The ranking in the search index is "mostly correct" -- it may lag by up to a factor of 2 at any point, but for ad ranking purposes this is more than sufficient. This technique trades perfect accuracy for orders-of-magnitude write reduction. It is a specific application of approximate counting using [probabilistic data structures](../11-patterns/probabilistic-data-structures.md) principles.
...
### Database Scaling
[Cassandra](../03-storage/cassandra.md) natively supports horizontal scaling via [consistent hashing](../02-scalability/consistent-hashing.md) with virtual nodes. Adding a node automatically rebalances data. The aggregation table is partitioned by `ad_id`, ensuring even distribution (ad IDs have high cardinality).
...
- [Event-driven architecture (Lambda/Kappa, stream processing, Flink, Spark)](../05-messaging/event-driven-architecture.md)
- [Message queues (Kafka for event ingestion, consumer groups, partitioning)](../05-messaging/message-queues.md)
- [Probabilistic data structures (logarithmic counting for index updates)](../11-patterns/probabilistic-data-structures.md)
- [Cassandra (time-series aggregation storage, consistent hashing)](../03-storage/cassandra.md)
- [Consistent hashing (Cassandra scaling, Kafka partition distribution)](../02-scalability/consistent-hashing.md)
- [Back-of-envelope estimation (QPS, storage, peak calculations)](../01-fundamentals/back-of-envelope-estimation.md)
- [CQRS (separate read/write models for raw vs. aggregated data)](../05-messaging/cqrs.md)
- [Database replication (Cassandra replication for fault tolerance)](../03-storage/database-replication.md)
