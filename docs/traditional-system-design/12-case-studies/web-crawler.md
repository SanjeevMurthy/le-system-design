...
The URL frontier is the central queue of URLs waiting to be crawled. [Amazon SQS](../05-messaging/message-queues.md) is preferred over Kafka for this use case because:
...
With 10 billion URLs to track, storing the full URL set in memory or a database is impractical. A [Bloom filter](../11-patterns/probabilistic-data-structures.md) provides space-efficient set membership testing:
...
- **robots.txt**: Before crawling any page on a domain, the fetcher checks the cached `robots.txt` for that domain (cached in [Redis](../04-caching/redis.md) with a 24-hour TTL). Disallowed paths are skipped.
- **Per-domain rate limiting**: A [Redis-based rate limiter](../04-caching/redis.md) enforces a maximum request rate per domain (e.g., 1 request/second). The fetcher checks `domain_rate:{domain}` before issuing each request.

### Direct-to-S3 Content Storage
Crawled page content is stored in [S3](../03-storage/object-storage.md) for downstream indexing. Pages are keyed by a hash of the URL, enabling fast deduplication checks at the content level (two different URLs serving identical content).
...
This [exponential backoff](../08-resilience/circuit-breaker.md) pattern prevents the crawler from overwhelming a temporarily down server and gives transient failures time to resolve.

### 5.2 Bandwidth Math and Capacity Planning

This is a critical [back-of-envelope estimation](../01-fundamentals/back-of-envelope-estimation.md) for the crawler design.
...
At 12GB, the Bloom filter fits on a single large instance (e.g., r5.4xlarge with 128GB RAM). For crawls exceeding 100B URLs, the filter can be distributed across a [Redis cluster](../04-caching/redis.md) with each node handling a range of hash values. Alternatively, multiple independent Bloom filters can be used (one per URL domain prefix), reducing the per-filter size.

### Content Storage
[S3](../03-storage/object-storage.md) scales to exabytes. No manual sharding required. Crawled pages are keyed by `sha256(normalized_url)`, providing natural distribution across S3's internal partitions.
...
| Target server returns 5xx | Page not crawled | [Exponential backoff](../08-resilience/circuit-breaker.md) via SQS retry with increasing visibility timeout |
...
- [Probabilistic data structures (Bloom filters for URL dedup)](../11-patterns/probabilistic-data-structures.md)
- [Message queues (SQS for URL frontier, visibility timeout, DLQ)](../05-messaging/message-queues.md)
- [Circuit breaker (exponential backoff with jitter for failed fetches)](../08-resilience/circuit-breaker.md)
- [Back-of-envelope estimation (bandwidth math, pages/sec, fleet sizing)](../01-fundamentals/back-of-envelope-estimation.md)
- [Object storage (S3 for crawled page content)](../03-storage/object-storage.md)
- [Redis (robots.txt cache, domain rate limiter, DNS cache)](../04-caching/redis.md)
- [Search and indexing (downstream inverted index for crawled content)](../11-patterns/search-and-indexing.md)
- [Rate limiting (per-domain politeness enforcement)](../08-resilience/rate-limiting.md)
