...
Each routing tile contains an adjacency list (graph nodes = intersections, edges = road segments with weights). Tiles reference neighboring tiles and tiles at different resolution levels, enabling the routing algorithm to "zoom out" from local streets to highways and back. Routing tiles are stored in [S3](../03-storage/03-object-storage.md) and cached aggressively on routing service instances.

### Kafka-Based Location Stream for Traffic and Map Improvement

User location updates during navigation are batched on the client (every 15 seconds) and sent to the server. The location service publishes these updates to [Kafka](../05-messaging/01-message-queues.md), creating a stream that multiple downstream services consume:
...
This [event-driven architecture](../05-messaging/02-event-driven-architecture.md) decouples the location ingestion path from the consumers, allowing each to scale independently.
...
3. The ETA service recomputes ETAs for affected users and pushes updates via [WebSocket](../07-api-design/04-real-time-protocols.md).
...
[Cassandra](../03-storage/07-cassandra.md) is chosen for the location history table due to its write-optimized architecture and ability to handle 200K+ writes/sec with horizontal scaling.
...
A [time-series database](../03-storage/06-time-series-databases.md) is ideal for traffic data: append-only writes, time-range queries, and automatic downsampling of historical data.
...
Geocoding lookups are read-heavy with infrequent writes (address data changes rarely). [Redis](../04-caching/02-redis.md) provides sub-millisecond reads. A backing relational database stores the full geocoding dataset.
...
The traffic update service processes the location stream to compute road segment speeds. This is a classic [stream processing](../05-messaging/02-event-driven-architecture.md) workload implemented with Apache Flink:
...
| Routing service instance crash | In-flight route computations fail | [Load balancer](../02-scalability/01-load-balancing.md) health checks detect failure; client retries are handled by a different instance; routing tiles cached across multiple instances |
...
- [Cassandra](../03-storage/07-cassandra.md) is the right choice for location history: high write throughput, time-series data model, TTL-based expiration, and horizontal scaling.
...
- [Geospatial Indexing (geohash, S2 for tile addressing)](../11-patterns/06-geospatial-indexing.md)
- [CDN (edge caching for map tiles)](../04-caching/03-cdn.md)
- [Object Storage (S3 for tiles and routing data)](../03-storage/03-object-storage.md)
- [Message Queues (Kafka for location streaming)](../05-messaging/01-message-queues.md)
- [Event-Driven Architecture (stream processing for traffic)](../05-messaging/02-event-driven-architecture.md)
- [Cassandra (location history)](../03-storage/07-cassandra.md)
- [Redis (geocoding cache, navigation sessions)](../04-caching/02-redis.md)
- [Time-Series Databases (traffic data)](../03-storage/06-time-series-databases.md)
- [Caching Strategies (tile caching, routing tile caching)](../04-caching/01-caching.md)
- [Load Balancing (routing service)](../02-scalability/01-load-balancing.md)
- [Real-Time Protocols (WebSocket for adaptive ETA)](../07-api-design/04-real-time-protocols.md)
- [CAP Theorem (eventual consistency for traffic, strong for geocoding)](../01-fundamentals/05-cap-theorem.md)
- [Back-of-Envelope Estimation (tile storage, CDN bandwidth)](../01-fundamentals/07-back-of-envelope-estimation.md)
- [Sharding (Kafka partitioning, traffic processing by region)](../02-scalability/04-sharding.md)
...
| Geospatial indexing (geohash, quadtree, S2 comparison) | docs/patterns/06-geospatial-indexing.md, Alex Xu Vol 2 (ch02: Proximity Service) |
...
| Google S2 geometry for map tile serving and geofencing | docs/patterns/06-geospatial-indexing.md (Section: Google S2 Geometry) |
