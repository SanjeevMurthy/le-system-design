...
The data model dictates the engine. See [SQL Databases](../03-storage/sql-databases.md), [NoSQL Databases](../03-storage/nosql-databases.md), [DynamoDB](../03-storage/dynamodb.md), and [Cassandra](../03-storage/cassandra.md).
...
- **E-commerce order system**: Normalized SQL for orders, payments, and inventory (ACID required for double-booking prevention). Denormalized product catalog in MongoDB for flexible attributes (size, color, material vary by category). Redis for session and cart data. See [SQL Databases](../03-storage/sql-databases.md).
...
- **IoT telemetry ingestion**: Cassandra for high-volume sensor writes (append-only, 100K+ writes/sec). Time-series downsampling for long-term storage. See [Cassandra](../03-storage/cassandra.md).
...
- **Messaging (WhatsApp)**: DynamoDB with partition key = conversation_id, sort key = timestamp for ordered message retrieval. GSI for per-user inbox queries. DynamoDB Streams for CDC to notification services. See [DynamoDB](../03-storage/dynamodb.md).
...
- **Bad partition keys.** Using low-cardinality keys like `country` or `is_premium` in DynamoDB or Cassandra creates hot partitions. Good partition keys have high cardinality (user_id, device_id) and even distribution. See [Database Indexing](../03-storage/database-indexing.md).
...
- **Over-indexing.** Every index accelerates reads but penalizes writes. A table with twelve indexes will have slow inserts. Index only the columns that appear in WHERE, ORDER BY, and JOIN clauses of your hot queries. See [Database Indexing](../03-storage/database-indexing.md).
...
- [SQL Databases](../03-storage/sql-databases.md) -- the canonical entity-driven modeling engine with ACID transactions
- [NoSQL Databases](../03-storage/nosql-databases.md) -- document, key-value, wide-column, and graph models for query-driven design
- [DynamoDB](../03-storage/dynamodb.md) -- single-table design, partition keys, sort keys, and GSI patterns
- [Cassandra](../03-storage/cassandra.md) -- wide-column query-driven modeling with tunable consistency
- [Database Indexing](../03-storage/database-indexing.md) -- B-trees, hash indexes, and specialized indexes that accelerate data retrieval
- [Sharding](../02-scalability/sharding.md) -- horizontal data distribution that depends on partition key design
- [Database Replication](../03-storage/database-replication.md) -- read replicas and consistency models shaped by the data model
- [CQRS](../05-messaging/cqrs.md) -- separating read and write models for optimal performance on both paths
- [Event Sourcing](../05-messaging/event-sourcing.md) -- append-only event logs as an alternative to mutable state
