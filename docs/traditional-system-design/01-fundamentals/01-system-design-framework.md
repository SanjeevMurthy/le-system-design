...
- **Back-of-Envelope Estimation:** Quantitative math (QPS, storage, bandwidth) used as a design tool to validate or invalidate architectural choices. See [Back-of-Envelope Estimation](./07-back-of-envelope-estimation.md) for detailed methodology.
...
3. **Quantify non-functional requirements.** Every requirement needs a number. How many DAU? What P99 latency? What availability target? What consistency model? See [Availability and Reliability](./04-availability-reliability.md) for nines targets.
...
5. **State the CAP trade-off.** For distributed systems, declare upfront whether you favor Consistency or Availability during partitions. See [CAP Theorem](./05-cap-theorem.md).
...
- **Read-heavy bottleneck:** Add caching layers, read replicas, CDN. Cross-link to [Caching](../04-caching/01-caching.md).
- **Write-heavy bottleneck:** Introduce message queues, async processing, write-back patterns.
- **Data volume:** Apply [Sharding](../02-scalability/04-sharding.md) with appropriate shard key design.
- **Hot spots / Celebrity problem:** Use compound shard keys, N-cache strategy, or hybrid fan-out. Cross-link to [Fan-out](../11-patterns/01-fan-out.md).
- **Single points of failure:** Add redundancy at every layer -- [Load Balancing](../02-scalability/01-load-balancing.md), database replication, multi-AZ deployment.
- **Consistency challenges:** Choose between strong and eventual consistency per data path. Cross-link to [CAP Theorem](./05-cap-theorem.md).

### Phase 5: Strategic Estimation and Validation

Use [Back-of-Envelope Estimation](./07-back-of-envelope-estimation.md) as a design tool, not a perfunctory ritual.
...
- [Back-of-Envelope Estimation](./07-back-of-envelope-estimation.md) -- quantitative validation of design decisions
- [Scaling Overview](./03-scaling-overview.md) -- vertical vs. horizontal scaling philosophy
- [Availability and Reliability](./04-availability-reliability.md) -- nines targets and SLA/SLO definitions
- [CAP Theorem](./05-cap-theorem.md) -- consistency vs. availability trade-off
- [Load Balancing](../02-scalability/01-load-balancing.md) -- traffic distribution as the first scaling mechanism
- [Sharding](../02-scalability/04-sharding.md) -- data partitioning as a deep-dive target
