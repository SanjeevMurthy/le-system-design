...
- **No single point of failure:** No central coordinator (unlike ZooKeeper). Cross-link to [Microservices](../06-architecture/02-microservices.md) for centralized service discovery.
...
- [Availability and Reliability](./04-availability-reliability.md) -- the availability target that CAP trades against consistency
- [Sharding](../02-scalability/04-sharding.md) -- partitioning data across nodes where consistency models apply
- [Database Replication](../03-storage/05-database-replication.md) -- the replication strategies that implement consistency guarantees
- [Distributed Transactions](../08-resilience/03-distributed-transactions.md) -- 2PC and Saga patterns for cross-partition consistency
- [SQL Databases](../03-storage/01-sql-databases.md) -- ACID compliance as strong consistency
- [Cassandra](../03-storage/07-cassandra.md) -- tunable consistency in practice
...
- source/extracted/grokking/ch267-05-cap-theorem.md -- CAP theorem definition, three guarantees explained
