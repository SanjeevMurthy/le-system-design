...
- **Elastic Scaling:** The ability to automatically add or remove capacity based on real-time demand. This is the cloud-native evolution of horizontal scaling. See [Autoscaling](../02-scalability/02-autoscaling.md).
...
- **Database:** Vertical scaling for the primary (bigger instance) + horizontal scaling for reads (read replicas). When even the largest single primary is insufficient, add [Sharding](../02-scalability/04-sharding.md).
...
- **Cache:** Horizontal scaling via [Consistent Hashing](../02-scalability/03-consistent-hashing.md) across a Redis cluster. Each node owns a segment of the key space.
- **Message queues:** Horizontal scaling via partitions (Kafka topics with multiple partitions, SQS with multiple consumers).
- **CDN:** Horizontal scaling by definition -- edge nodes in 100+ geographic locations serving cached content. Cross-link to [CDN](../04-caching/03-cdn.md).
...
| **Cache (Redis)** | Bigger instance with more RAM | Redis Cluster with hash slots | Hot key problem; see [Consistent Hashing](../02-scalability/03-consistent-hashing.md) |
...
Cross-link to [Microservices](../06-architecture/02-microservices.md) for detailed patterns.
...
- **Over-provisioning.** Keeping 50 servers running for a workload that needs 5 is wasteful. Elastic scaling (auto-scaling) is the answer -- see [Autoscaling](../02-scalability/02-autoscaling.md).
...
- [Load Balancing](../02-scalability/01-load-balancing.md) -- the mechanism that makes horizontal scaling work
- [Autoscaling](../02-scalability/02-autoscaling.md) -- elastic scaling based on real-time metrics
- [Sharding](../02-scalability/04-sharding.md) -- horizontal scaling for databases
- [Consistent Hashing](../02-scalability/03-consistent-hashing.md) -- data distribution for horizontally scaled systems
- [Availability and Reliability](./04-availability-reliability.md) -- why horizontal scaling improves fault tolerance
- [Back-of-Envelope Estimation](./07-back-of-envelope-estimation.md) -- determining when to scale
