...
- **Read-heavy bottleneck:** Add caching layers, read replicas, CDN. Cross-link to [Caching](../04-caching/caching.md).
- **Write-heavy bottleneck:** Introduce message queues, async processing, write-back patterns.
- **Data volume:** Apply [Sharding](../02-scalability/sharding.md) with appropriate shard key design.
- **Hot spots / Celebrity problem:** Use compound shard keys, N-cache strategy, or hybrid fan-out. Cross-link to [Fan-out](../11-patterns/fan-out.md).
- **Single points of failure:** Add redundancy at every layer -- [Load Balancing](../02-scalability/load-balancing.md), database replication, multi-AZ deployment.
...
- [Load Balancing](../02-scalability/load-balancing.md) -- traffic distribution as the first scaling mechanism
- [Sharding](../02-scalability/sharding.md) -- data partitioning as a deep-dive target
