...
- [Logging](logging.md) -- logs provide event-level detail that metrics aggregate away; the two are complementary
- [Autoscaling](../02-scalability/autoscaling.md) -- HPA scales based on metrics; accurate monitoring is a prerequisite for effective autoscaling
- [Availability and Reliability](../01-fundamentals/availability-reliability.md) -- SLAs, SLOs, and nines of availability are defined and measured through monitoring
- [Time-Series Databases](../03-storage/time-series-databases.md) -- the storage engines underlying monitoring systems (LSM trees, Gorilla compression, downsampling)
- [Circuit Breaker](../08-resilience/circuit-breaker.md) -- circuit breaker state transitions are driven by error rate metrics
