...
- **Confusing consistent hashing with caching.** Consistent hashing is a data distribution strategy, not a caching strategy. It determines where data lives, not how long it stays. TTL, eviction policies, and cache-aside logic are separate concerns. Cross-link to [Caching](../04-caching/caching.md).
...
- [Cassandra](../03-storage/cassandra.md) -- consistent hashing ring as Cassandra's core architecture
- [DynamoDB](../03-storage/dynamodb.md) -- consistent hashing for partition key distribution
- [Redis](../04-caching/redis.md) -- 16,384 hash slots in Redis Cluster
- [Caching](../04-caching/caching.md) -- consistent hashing for distributed cache clusters
