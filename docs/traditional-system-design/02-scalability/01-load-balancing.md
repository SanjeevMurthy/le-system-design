...
- **Reverse Proxy:** A server-side intermediary that receives requests from clients and forwards them to backend servers. Load balancers function as reverse proxies. See [Networking Fundamentals](../01-fundamentals/06-networking-fundamentals.md) for proxy details.
...
- **Ticketmaster:** During surge events (Taylor Swift ticket sales), load balancers work with virtual waiting rooms to throttle ingress and prevent backend collapse. Health checks aggressively remove struggling servers. Cross-link to [Rate Limiting](../08-resilience/01-rate-limiting.md).
...
- [Networking Fundamentals](../01-fundamentals/06-networking-fundamentals.md) -- OSI layers, TCP/UDP, reverse proxy details
- [Autoscaling](./02-autoscaling.md) -- dynamically adjusting the server pool behind the load balancer
- [Consistent Hashing](./03-consistent-hashing.md) -- distribution algorithm used in advanced load balancing
- [API Gateway](../06-architecture/01-api-gateway.md) -- application-layer routing, authentication, rate limiting
- [Availability and Reliability](../01-fundamentals/04-availability-reliability.md) -- load balancing as an availability mechanism
- [Rate Limiting](../08-resilience/01-rate-limiting.md) -- traffic throttling often implemented at the LB layer
...
- source/extracted/grokking/ch243-benefits-of-01-load-balancing.md -- LB placement (3 layers), benefits
