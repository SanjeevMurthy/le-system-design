...
- **Ignoring exponential backoff on the client**: Rate-limited clients must implement exponential backoff with jitter. Without jitter, all throttled clients retry at the exact same time, creating a thundering herd. See [Circuit Breaker](./circuit-breaker.md).
...
- [API Gateway](../06-architecture/api-gateway.md) -- the most common placement point for rate limiting
- [Circuit Breaker](./circuit-breaker.md) -- exponential backoff and jitter for retry storms (canonical detail there)
- [Redis](../04-caching/redis.md) -- the standard backing store for rate limit state (atomic INCR, Lua scripts)
- [Feature Flags](./feature-flags.md) -- can be used to dynamically adjust rate limits during incidents
- [Load Balancing](../02-scalability/load-balancing.md) -- load balancers can enforce basic rate limits at L7
