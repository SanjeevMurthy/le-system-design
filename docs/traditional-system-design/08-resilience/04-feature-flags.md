...
- [API Gateway](../06-architecture/01-api-gateway.md) -- gateways can evaluate flags to route traffic to different backends
- [Rate Limiting](./01-rate-limiting.md) -- feature flags can dynamically adjust rate limits during incidents
- [Circuit Breaker](./02-circuit-breaker.md) -- kill switches are a manual circuit breaker; both protect against failure
- [Autoscaling](../02-scalability/02-autoscaling.md) -- feature flags can disable expensive features as an alternative to scaling
- [Monitoring](../10-observability/01-monitoring.md) -- flag changes should be correlated with metric changes for root cause analysis
