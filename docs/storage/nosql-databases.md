...
- **Eventual consistency**: Many NoSQL systems favor availability over immediate consistency (AP in CAP theorem). See [CAP Theorem](../fundamentals/cap-theorem.md).
...
The correct choice depends on the specific operation, not the entire system. A banking transfer needs strong consistency; a social media "likes" count tolerates eventual consistency. See [CAP Theorem](../fundamentals/cap-theorem.md) for a deeper treatment.
...
- **Redis at Twitter**: In-memory cache for home timelines, session storage, and rate limiting. See [Redis](../caching/redis.md).
...
- [Redis](../caching/redis.md) --- in-memory key-value store for caching and data structures
