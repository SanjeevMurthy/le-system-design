...
**Pre-signed URLs**: For uploads exceeding 5MB, the client obtains a [pre-signed URL](../storage/object-storage.md) from the upload service and uploads directly to S3, bypassing the application server. This prevents the app server from becoming a bottleneck during traffic spikes and enables [multipart uploads](../storage/object-storage.md) for large video files.
...
1. **Pre-computation (write path)**: When a user publishes a post, a fan-out service consumes the event from Kafka and writes the post_id into each follower's feed list in [Redis](../caching/redis.md). For celebrity accounts (> 10K followers), the fan-out is skipped to avoid write amplification. The pre-computed feed is stored as a Redis sorted set, keyed by `feed:{user_id}` with the post timestamp as the score.
...
**Storage**: Stories are stored in [Cassandra](../storage/cassandra.md) with a TTL of 24 hours (plus a grace period for archival to "Highlights"). The partition key is `author_id`, and the clustering key is `created_at`, enabling efficient retrieval of all active stories from a single author in chronological order.

**View tracking**: A separate Cassandra table tracks `(viewer_id, story_id)` pairs. This write-heavy workload (potentially billions of view events per day) is well-suited to Cassandra's append-optimized write path. The view status is cached in [Redis](../caching/redis.md) with a 24-hour TTL to avoid redundant DB reads when rendering the Stories tray.
...
The Explore page surfaces content from accounts the user does not follow. This is a large-scale [recommendation problem](../patterns/recommendation-engines.md) requiring evaluation of billions of candidate posts.
...
1. **Candidate generation**: Multiple candidate sources run in parallel:
   ...
   - *Content-based*: Posts semantically similar to posts the user has recently engaged with (using [vector embeddings and ANN search](../patterns/recommendation-engines.md)).
...
**Indexing**: Explore relies on a real-time content index maintained via [CDC from the metadata DB to Elasticsearch](../patterns/search-and-indexing.md). New posts are indexable for Explore within seconds of upload completion.
...
Image processing is CPU and memory intensive. Workers are horizontally scaled via [Kafka consumer groups](../messaging/message-queues.md) -- each partition maps to a worker, and adding partitions/workers increases throughput linearly. At 100M uploads/day (~1,150/sec), with 4-6 variants per image and ~5 seconds of processing per variant, the system requires approximately 5,750-8,600 concurrent processing slots. [Autoscaling](../scalability/autoscaling.md) dynamically adjusts the worker fleet based on Kafka consumer lag.

### Feed Service Scaling
The feed service is the most latency-sensitive read path. It is stateless and scaled horizontally behind a [load balancer](../scalability/load-balancing.md). The Redis feed cache is clustered across 16,384 slots using [consistent hashing](../scalability/consistent-hashing.md). Hot keys (popular users' feeds receiving millions of reads during viral events) are mitigated by replicating hot keys across multiple cache replicas.

### Social Graph Sharding
The follows table grows linearly with user count and is read-heavy (every feed generation requires a social graph lookup). It is [sharded](../scalability/sharding.md) by `follower_id`, ensuring that all of a user's followed accounts are co-located on a single shard for efficient lookup. Read replicas serve the fan-out service's queries, keeping write traffic isolated on the primary.
...
- [Autoscaling](../scalability/autoscaling.md) feed and upload services based on traffic forecasts.
- Shedding non-critical work (deferring analytics processing, reducing Explore candidate pool) during peak load.
- [Rate limiting](../resilience/rate-limiting.md) at the API gateway to prevent abuse and protect downstream services.

## 8. Failure Modes & Mitigations

| Failure | Impact | Mitigation |
|---------|--------|------------|
| Image processing worker crash | Photo stuck in "processing" status; user sees placeholder | Kafka consumer offset ensures replay; dead-letter queue captures poison messages; [circuit breaker](../resilience/circuit-breaker.md) prevents cascading failures |
| Redis feed cache node failure | Feed reads for affected users return empty or stale | Redis cluster auto-failover redistributes slots within seconds; [cache-aside](../caching/caching.md) falls through to DB; feed is reconstructed on next access |
| S3 origin outage | CDN cache misses fail; newly uploaded photos are inaccessible | CDN serves stale cached copies (graceful degradation); multi-region S3 replication provides failover; upload service queues requests with [back-pressure](../messaging/message-queues.md) |
...
| Celebrity posts viral spike | Massive surge in feed read-time merges and media requests | Celebrity posts are cached with short TTL (30s); CDN absorbs media read surge; [rate limiting](../resilience/rate-limiting.md) on feed API throttles excessive requests |
...
- [Fan-out (read vs. write, hybrid)](../patterns/fan-out.md) -- feed generation strategy
- [Object storage (S3, pre-signed URLs, multipart uploads)](../storage/object-storage.md) -- media storage
- [CDN (edge caching, origin pull, cache invalidation)](../caching/cdn.md) -- media delivery
- [Redis (sorted sets, clustering, hot key mitigation)](../caching/redis.md) -- feed cache and Stories cache
- [SQL databases (PostgreSQL, ACID, indexing)](../storage/sql-databases.md) -- metadata and social graph
- [Cassandra (TTL, partition keys, LSM writes)](../storage/cassandra.md) -- interactions and Stories
- [Message queues (Kafka, consumer groups, back-pressure)](../messaging/message-queues.md) -- async processing pipeline
- [Recommendation engines (three-stage pipeline, embeddings, ANN)](../patterns/recommendation-engines.md) -- Explore and feed ranking
- [Search and indexing (Elasticsearch, CDC-based sync)](../patterns/search-and-indexing.md) -- account and hashtag search
- [Sharding (hash-based partitioning, shard key design)](../scalability/sharding.md) -- database scaling
- [Consistent hashing (Redis cluster slots)](../scalability/consistent-hashing.md) -- cache distribution
- [Caching strategies (cache-aside, TTL, cache warming)](../caching/caching.md) -- multi-tier caching
- [Rate limiting (API protection)](../resilience/rate-limiting.md) -- traffic spike protection
- [Autoscaling (worker fleet, feed service)](../scalability/autoscaling.md) -- elastic capacity
- [Load balancing (L7, request routing)](../scalability/load-balancing.md) -- traffic distribution
- [API gateway (routing, auth, rate limiting)](../architecture/api-gateway.md) -- entry point
- [Microservices (service decomposition)](../architecture/microservices.md) -- service architecture
- [Event-driven architecture (async processing, pub/sub)](../messaging/event-driven-architecture.md) -- processing pipeline
- [Circuit breaker (cascading failure prevention)](../resilience/circuit-breaker.md) -- resilience
- [Database replication (leader-follower, CDC)](../storage/database-replication.md) -- HA and search sync
- [Real-time protocols (WebSocket for DMs)](../api-design/real-time-protocols.md) -- direct messaging
