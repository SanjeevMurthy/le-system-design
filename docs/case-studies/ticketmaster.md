...
1. **Reserve**: A temporary hold is placed on the seat using a [distributed lock in Redis](../caching/redis.md). The lock has a TTL (e.g., 10 minutes).
...
Payment operations include an idempotency key (the `reservation_id`). If the client retries a payment request due to a network timeout, the payment provider recognizes the duplicate idempotency key and returns the original result rather than charging twice. This is essential for financial operations -- see [distributed transactions](../resilience/distributed-transactions.md) for the saga pattern's compensating transaction model.
...
1. **Approximate view**: Served from a CDN-cached or [Redis-cached](../caching/redis.md) snapshot updated every 10-30 seconds. Users see sections with approximate availability ("~50 seats remaining in Section A").
...
[CDN edge caching](../caching/cdn.md) absorbs the majority of search traffic during surges. Behind the CDN, Elasticsearch provides fast full-text search with horizontal shard scaling. Event data is indexed via [CDC](../storage/database-replication.md) from the event DB, keeping the search index eventually consistent without polluting the write path.
...
### Database Scaling
The seat inventory table is [sharded](../scalability/sharding.md) by `event_id`. Each event's seats live on a single shard, allowing row-level locking within a single partition. Cross-event transactions are not needed -- a user never books seats across different events atomically.
...
- Additional booking service instances are spun up via [autoscaling](../scalability/autoscaling.md) group configuration changes.
...
| Waiting room failure | All users flood the backend simultaneously | [Circuit breaker](../resilience/circuit-breaker.md) at API gateway returns 503 with retry-after header; auto-scaling kicks in |
...
- Strong consistency is non-negotiable for inventory systems. [SQL with ACID transactions](../storage/sql-databases.md) is the correct choice for the booking table. Double-booking is a catastrophic business failure, not just a technical issue.
...
- [Distributed transactions (two-phase commit, saga pattern)](../resilience/distributed-transactions.md)
- [Redis (TTL locks, distributed locking, SET NX)](../caching/redis.md)
- [Rate limiting (virtual waiting room, queue-based admission)](../resilience/rate-limiting.md)
- [SQL databases (ACID, row-level locking, Postgres)](../storage/sql-databases.md)
- [CDN (edge caching for search)](../caching/cdn.md)
- [Sharding (event_id-based partitioning)](../scalability/sharding.md)
- [Circuit breaker (fail-fast during overload)](../resilience/circuit-breaker.md)
- [Search and indexing (Elasticsearch for event search)](../patterns/search-and-indexing.md)
- [Load balancing (L7 routing to booking service)](../scalability/load-balancing.md)
- [Consistent hashing (Redis cluster slot distribution)](../scalability/consistent-hashing.md)
