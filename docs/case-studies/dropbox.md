...
This is an application of [multipart uploads](../storage/object-storage.md), which are native to S3 and similar object storage services.
...
To prevent the API service from becoming a bandwidth bottleneck, the upload flow uses [pre-signed URLs](../storage/object-storage.md):
...
For high-priority sync (e.g., active collaboration), the system can upgrade to [long polling or WebSocket](../api-design/real-time-protocols.md) connections where the server pushes change notifications immediately.
...
### Object Storage
[S3](../storage/object-storage.md) scales horizontally and provides 11 nines of durability via erasure coding across racks and data centers. No manual sharding is needed. S3's flat namespace and partition-prefix strategy handle the 10B+ chunk objects without performance degradation.
...
### Metadata DB
[Postgres](../storage/sql-databases.md) is [sharded](../scalability/sharding.md) by `user_id`. Each user's file metadata lives on a single shard, enabling ACID operations (rename, move, delete) within a single partition. The chunk registry (fingerprint -> S3 key mapping) is a separate table that can be sharded by fingerprint, since it is accessed independently.
...
- [Object storage (S3, pre-signed URLs, multipart uploads, erasure coding)](../storage/object-storage.md)
- [Caching strategies (content-addressable dedup, cache warming)](../caching/caching.md)
- [Message queues (Kafka for sync event propagation)](../messaging/message-queues.md)
- [SQL databases (Postgres for ACID file metadata)](../storage/sql-databases.md)
- [Sharding (user_id-based metadata partitioning)](../scalability/sharding.md)
- [Real-time protocols (long polling, WebSocket for sync notifications)](../api-design/real-time-protocols.md)
- [Database replication (leader-follower for metadata DB)](../storage/database-replication.md)
- [Back-of-envelope estimation (storage, bandwidth calculations)](../fundamentals/back-of-envelope-estimation.md)
