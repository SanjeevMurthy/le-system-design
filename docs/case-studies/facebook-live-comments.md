...
### SSE Over WebSockets
Live comments are a fundamentally **unidirectional** stream -- the server pushes comments to the client. The client's only "write" action (posting a comment) is a standard HTTP POST, not a persistent bidirectional channel. [Server-Sent Events (SSE)](../api-design/real-time-protocols.md) is the preferred protocol because:
...
The solution is to partition the [Redis pub/sub](../caching/redis.md) layer using `hash(videoID) % N`:
...
- **Horizontal scaling**: New connection servers are added behind the [load balancer](../scalability/load-balancing.md). When a viewer connects, the LB assigns them to the least-loaded server.
...
3. **Rate limiting**: Users are limited to N comments per minute per video (e.g., 10 comments/minute) via a [Redis-based rate limiter](../caching/redis.md). This prevents spam bots from flooding a popular video.
...
Using [HyperLogLog](../patterns/probabilistic-data-structures.md) for viewer counts provides bounded memory usage (~12KB per video) regardless of whether the video has 100 viewers or 100M viewers. The ~1% error is invisible in the UI ("1.2M viewers" vs "1.21M viewers").
...
Connection servers are deployed in multiple regions (US-East, US-West, EU, APAC). Viewers connect to the nearest region via DNS-based [load balancing](../scalability/load-balancing.md). Comments posted in one region are published to the local Redis instance and replicated to other regions' Redis instances via a cross-region pub/sub bridge. This adds ~50-100ms of latency for cross-region viewers, which is within the 200ms total budget.

### Dynamic Scaling During Viral Events
When a video goes viral, the system auto-scales:
1. [Autoscaling](../scalability/autoscaling.md) groups detect increased connection counts and spin up additional connection servers.
...
| Comment service overload | Comments are rejected or delayed | [Rate limiting](../resilience/rate-limiting.md) per user; back-pressure via message queue between API and comment service |
...
- Eventual consistency is explicitly acceptable for live comments -- users tolerate missing or slightly delayed comments in exchange for a responsive, high-availability experience. This is an AP choice per the [CAP theorem](../fundamentals/cap-theorem.md).
...
- [Real-time protocols (SSE vs. WebSockets, reconnection semantics)](../api-design/real-time-protocols.md)
- [Redis (pub/sub, channel partitioning, fire-and-forget delivery)](../caching/redis.md)
- [Fan-out (server-side fan-out to connected clients)](../patterns/fan-out.md)
- [Load balancing (distributing SSE connections across servers)](../scalability/load-balancing.md)
- [Rate limiting (protecting comment service from abuse)](../resilience/rate-limiting.md)
- [Message queues (Kafka as alternative for durable comment streams)](../messaging/message-queues.md)
- [CAP theorem (AP choice for live comments)](../fundamentals/cap-theorem.md)
- [CDN (edge deployment of connection servers)](../caching/cdn.md)
