...
Standard 1D B-tree indexes in SQL databases fail for 2D proximity queries because latitude and longitude are independent axes. Tinder employs [geospatial indexing](../patterns/geospatial-indexing.md) using one of two approaches:
...
Swipes are high-velocity, append-only writes (2B+/day). [Cassandra](../storage/cassandra.md) is purpose-built for this pattern -- its append-only commit log + MemTable + SSTable architecture avoids the disk-seek overhead of relational databases. The partition key is `user_id`, with the clustering key being `swiped_user_id`, enabling efficient lookups like "did User A already swipe on User B?"

### Atomic Match Detection in Redis
To prevent race conditions during simultaneous mutual swipes, Tinder uses atomic operations in [Redis](../caching/redis.md). When User A swipes right on User B, the swipe service atomically checks if B has already swiped right on A. Because Redis is single-threaded, the check-and-write operation is inherently serialized, eliminating the race condition that would occur with a distributed NoSQL read-then-write.

### Bloom Filters for Repeat Profile Prevention
To avoid showing users profiles they have already swiped on, the system maintains a per-user [Bloom filter](../patterns/probabilistic-data-structures.md) in Redis. A Bloom filter returns "definitely not seen" or "probably seen" -- the false positive rate means a small percentage of valid profiles are incorrectly skipped, but no already-seen profile is ever reshown. This is a massive space saving compared to storing the complete set of swiped user IDs.
...
4. **Ranking**: Remaining candidates are ranked by a scoring algorithm (profile completeness, activity recency, mutual interest signals from the [recommendation engine](../patterns/recommendation-engines.md)).
...
Tinder's ranking algorithm is a simplified version of a [recommendation engine](../patterns/recommendation-engines.md):
...
1. User uploads photo via [pre-signed URL](../storage/object-storage.md) to S3.
...
The geospatial index is [sharded](../scalability/sharding.md) by geographic region (geohash prefix). Dense urban areas (Manhattan, Tokyo) have more shards than sparse rural areas. This mirrors the quad-tree's dynamic splitting behavior -- more resolution where more users exist.

The index must handle both high read volume (discovery queries) and high write volume (location updates). Reads and writes are separated: writes update a primary geospatial store, while reads query a replicated read-optimized copy. This is an application of [CQRS](../messaging/cqrs.md) at the data layer.

### Swipe Write Throughput
[Cassandra's](../storage/cassandra.md) append-only architecture handles 10B+ swipes/day across a modest cluster. Writes hit the commit log and MemTable and return immediately -- no disk seeks on the write path. Cassandra's tunable consistency is set to `ONE` for swipe writes (fast, best-effort) since swipe data loss for a single swipe is a minor UX issue, not a critical failure.
...
### Photo Delivery
Profile photos are served via [CDN](../caching/cdn.md). With 100M users x 5 photos avg = 500M photos, CDN edge caching is essential. Photos are uploaded via [pre-signed URLs](../storage/object-storage.md) to S3 and served through CloudFront or equivalent. The CDN cache hit ratio for frequently viewed profiles (those with high Elo scores shown to many users) approaches 99%.

### Location Updates
Users update their location on every app open and periodically while browsing. At 100M DAU x ~10 location updates/day = 1B location writes/day (~11.5K writes/sec). These are batched and written to the geospatial index via a [Kafka](../messaging/message-queues.md) queue, smoothing out spikes when millions of users open the app simultaneously (e.g., evening commute hours).
...
- [Geospatial indexing (geohashing, quad-trees, R-trees)](../patterns/geospatial-indexing.md)
- [Redis (atomic ops, bloom filter storage, TTL, pub/sub)](../caching/redis.md)
- [Probabilistic data structures (Bloom filters)](../patterns/probabilistic-data-structures.md)
- [Cassandra (append-only writes, partition key, clustering key)](../storage/cassandra.md)
- [Recommendation engines (candidate generation, ranking)](../patterns/recommendation-engines.md)
- [CDN (photo delivery at scale)](../caching/cdn.md)
- [Consistent hashing (Redis cluster, geospatial shard distribution)](../scalability/consistent-hashing.md)
- [Sharding (geographic partitioning)](../scalability/sharding.md)
- [Message queues (async swipe processing)](../messaging/message-queues.md)
- [Real-time protocols (WebSocket for match notifications)](../api-design/real-time-protocols.md)
