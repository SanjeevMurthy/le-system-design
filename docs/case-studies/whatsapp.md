...
- **Security**: End-to-end [encryption](../security/encryption.md) -- the server never sees plaintext message content.
...
Unlike live comments (unidirectional, SSE), messaging requires the client to both send and receive messages over the same persistent connection. [WebSockets](../api-design/real-time-protocols.md) provide full-duplex communication over a single TCP connection. Each user's device maintains one WebSocket connection to a connection server.

### Redis Pub/Sub for Message Routing
When User A sends a message to User B, the message service must route it to the specific connection server holding User B's WebSocket. [Redis pub/sub](../caching/redis.md) provides this routing layer:
...
The system uses [ZooKeeper (service discovery)](../architecture/microservices.md) to maintain a mapping of `deviceID -> connectionServerHost`. When a device connects, the connection server registers the mapping. When the message service needs to route a message, it looks up the recipient's host via ZooKeeper.
...
**Implementation:**
- Each connection server periodically reports its connected users' status to a shared [Redis](../caching/redis.md) instance.
...
**Scaling consideration**: Presence updates are high-frequency (one heartbeat per connection per 10-30 seconds). For 100M concurrent users, this is 3-10M writes/sec to Redis. The presence store is sharded across multiple Redis nodes using [consistent hashing](../scalability/consistent-hashing.md).
...
WhatsApp uses the Signal Protocol for [end-to-end encryption](../security/encryption.md):
...
### Connection Server Fleet
Each server handles ~200K concurrent WebSocket connections. For 500M concurrent users at peak, the fleet needs ~2,500 connection servers. Connection servers are stateful (they hold WebSocket sessions), making them harder to scale than stateless services. [ZooKeeper](../architecture/microservices.md) manages host assignment, and ephemeral nodes provide automatic cleanup when a server fails.
...
Groups with 1,000 members create 1,000 inbox writes and 1,000 pub/sub publishes. This is handled asynchronously via [Kafka](../messaging/message-queues.md) to avoid blocking the sender's response. The sender receives a 200 OK as soon as the message is persisted to Kafka. Consumer workers then perform the fan-out to each member's inbox and pub/sub channel.
...
- [Real-time protocols (WebSockets for bidirectional communication)](../api-design/real-time-protocols.md)
- [Redis (pub/sub for message routing, presence with TTL)](../caching/redis.md)
- [Message queues (Kafka for async group fan-out)](../messaging/message-queues.md)
- [Cassandra (inbox store, high write throughput)](../storage/cassandra.md)
- [Microservices (ZooKeeper for service discovery / connection mapping)](../architecture/microservices.md)
- [Encryption (end-to-end encryption for message privacy)](../security/encryption.md)
- [Consistent hashing (Redis cluster sharding for presence)](../scalability/consistent-hashing.md)
- [CAP theorem (eventual consistency for presence, at-least-once for messages)](../fundamentals/cap-theorem.md)
- [Circuit breaker (handling downstream failures gracefully)](../resilience/circuit-breaker.md)
