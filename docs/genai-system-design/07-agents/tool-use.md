...
- **Structured output** (co-located): Tool calls are a form of structured output --- the model must emit valid function names and typed arguments. See [Structured Output](../06-prompt-engineering/structured-output.md).
...
- **Orchestration frameworks** (infrastructure): LangChain, LlamaIndex, and Semantic Kernel provide tool abstractions and registries. See [Orchestration Frameworks](../08-orchestration/orchestration-frameworks.md).
- **Prompt injection** (security): Tools that access external data (web search, file read) are vectors for indirect prompt injection. See [Prompt Injection](../06-prompt-engineering/prompt-injection.md).
...
- **[Structured Output](../06-prompt-engineering/structured-output.md)**: Tool calls are a form of structured output --- guaranteed schema conformance is critical for reliable tool use.
- **[Prompt Injection](../06-prompt-engineering/prompt-injection.md)**: Tools that fetch external content are vectors for indirect prompt injection attacks.
- **[Orchestration Frameworks](../08-orchestration/orchestration-frameworks.md)**: LangChain, LlamaIndex, and Semantic Kernel provide tool abstractions, registries, and execution runtimes.
- **[Prompt Patterns](../06-prompt-engineering/prompt-patterns.md)**: System prompt design for tool-using agents, including tool descriptions and few-shot examples.
- **[Context Management](../06-prompt-engineering/context-management.md)**: Managing tool schemas and results within the LLM's context window budget.
- **[Model Serving](../02-llm-architecture/model-serving.md)**: How tool calls are processed at the LLM inference layer, including constrained decoding.
