...
- **Agent architecture** (foundation): Code agents implement ReAct/tool-use patterns, with tools specialized for code operations (file read/write, terminal execution, LSP queries). See [Agent Architecture](./01-agent-architecture.md).
- **Tool use** (capability): The agent's tool set defines its capability boundary — file I/O, shell execution, browser, API calls. See [Tool Use](./02-tool-use.md).
- **Copilot architecture** (deployment): Production code agents are deployed as IDE extensions, CLI tools, or cloud services with specific latency and UX requirements. See [Copilot Architecture](../13-case-studies/02-copilot-architecture.md).
- **Eval frameworks** (quality): Code agent quality is measured through pass@k, SWE-bench, and task-specific benchmarks. See [Eval Frameworks](../09-evaluation/01-eval-frameworks.md).
- **Memory systems** (context): Code agents use codebase indexing as a form of semantic memory and conversation history as short-term memory. See [Memory Systems](./03-memory-systems.md).
...
- [Agent Architecture](./01-agent-architecture.md) — Foundational agent patterns (ReAct, function calling) used by code agents.
- [Tool Use](./02-tool-use.md) — Tool design for code agents: file I/O, shell execution, LSP queries, browser.
- [Copilot Architecture](../13-case-studies/02-copilot-architecture.md) — Deployment patterns for code assistance products (IDE extension, CLI, cloud).
- [Eval Frameworks](../09-evaluation/01-eval-frameworks.md) — Benchmarks for code agents: SWE-bench, HumanEval, MBPP, Aider benchmark.
- [Memory Systems](./03-memory-systems.md) — Codebase indexing as semantic memory; conversation history for multi-turn coding sessions.
- [Multi-Agent Systems](./04-multi-agent.md) — Multi-agent patterns in code agents (planner + coder + reviewer).
- [Context Management](../06-prompt-engineering/03-context-management.md) — Token budgeting strategies for code context.
