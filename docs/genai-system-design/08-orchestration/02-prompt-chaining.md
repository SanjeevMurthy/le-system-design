...
- **Orchestration frameworks** (runtime): LangChain LCEL, LlamaIndex query engines, Haystack pipelines, and DSPy modules all provide chain execution runtimes. See [Orchestration Frameworks](./01-orchestration-frameworks.md).
- **Prompt patterns** (individual steps): Each chain step uses prompt patterns (zero-shot, few-shot, CoT, structured output) internally. See [Prompt Patterns](../06-prompt-engineering/01-prompt-patterns.md).
- **Agent architecture** (dynamic chaining): Agents are chains where the sequence of steps is determined dynamically by the LLM, not statically by the developer. See [Agent Architecture](../07-agents/01-agent-architecture.md).
- **Model routing** (step-level model selection): Different chain steps may use different models based on cost/quality tradeoffs. See [Model Selection](../03-model-strategies/01-model-selection.md).
- **RAG pipeline** (retrieval steps): RAG is a specific chain pattern: query transform -> retrieve -> rerank -> generate. See [RAG Pipeline](../04-rag/01-rag-pipeline.md).
- **Structured output** (inter-step contracts): JSON schemas and Pydantic models enforce the data contract between chain steps. See [Structured Output](../06-prompt-engineering/02-structured-output.md).
...
- **[Orchestration Frameworks](./01-orchestration-frameworks.md)**: The frameworks (LangChain, LlamaIndex, Haystack, DSPy) that provide runtime implementations for prompt chains.
- **[Prompt Patterns](../06-prompt-engineering/01-prompt-patterns.md)**: Individual prompt techniques (CoT, few-shot, ReAct) used within each chain step.
- **[Agent Architecture](../07-agents/01-agent-architecture.md)**: Agents as dynamically composed chains where the LLM determines the chain structure at runtime.
- **[Model Selection](../03-model-strategies/01-model-selection.md)**: How to choose different models for different chain steps based on cost/quality/latency requirements.
- **[RAG Pipeline](../04-rag/01-rag-pipeline.md)**: RAG as a specific instantiation of the sequential chain pattern (query transform -> retrieve -> rerank -> generate).
- **[Structured Output](../06-prompt-engineering/02-structured-output.md)**: JSON schemas and constrained decoding for reliable inter-step data contracts.
- **[Context Management](../06-prompt-engineering/03-context-management.md)**: Managing conversation history and context across chain steps and multi-turn interactions.
- **[Build vs Buy](./03-build-vs-buy.md)**: When to use framework-provided chain composition vs. custom implementation.
