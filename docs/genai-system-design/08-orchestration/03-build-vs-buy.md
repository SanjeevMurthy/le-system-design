...
- **Orchestration frameworks** (the "buy" options): LangChain, LlamaIndex, Haystack, Semantic Kernel, DSPy each represent a different point on the abstraction spectrum. See [Orchestration Frameworks](./01-orchestration-frameworks.md).
- **Prompt chaining** (composition patterns): Whether you implement chains via LCEL, Haystack pipelines, or custom async functions depends on this decision. See [Prompt Chaining](./02-prompt-chaining.md).
- **RAG pipeline** (retrieval integration): Framework retriever abstractions vs. direct vector DB client calls. The retrieval hot path is often the first component extracted from a framework. See [RAG Pipeline](../04-rag/01-rag-pipeline.md).
- **Agent architecture** (agent runtime): LangGraph vs. custom agent loops. Agent state management is where framework value is highest (checkpointing, human-in-the-loop, cycle support). See [Agent Architecture](../07-agents/01-agent-architecture.md).
- **Model serving** (inference layer): Frameworks wrap LLM API calls. At high throughput, the wrapper overhead matters. See [Model Serving](../02-llm-architecture/01-model-serving.md).
- **Evaluation** (quality assurance): Framework-integrated evaluation (LangSmith, deepset Cloud) vs. custom eval pipelines (RAGAS + custom metrics). See [Eval Frameworks](../09-evaluation/01-eval-frameworks.md).
...
- **[Orchestration Frameworks](./01-orchestration-frameworks.md)**: Deep technical coverage of each framework (LangChain, LlamaIndex, Haystack, Semantic Kernel, DSPy) that this document compares.
- **[Prompt Chaining](./02-prompt-chaining.md)**: The composition patterns that frameworks implement and that custom code must replicate.
- **[RAG Pipeline](../04-rag/01-rag-pipeline.md)**: RAG-specific build-vs-buy considerations, especially for retrieval and indexing components.
- **[Agent Architecture](../07-agents/01-agent-architecture.md)**: Agent-specific framework considerations, especially LangGraph vs. custom agent loops.
- **[Model Serving](../02-llm-architecture/01-model-serving.md)**: The inference layer that all frameworks and custom implementations ultimately call.
- **[Eval Frameworks](../09-evaluation/01-eval-frameworks.md)**: Evaluation tooling decisions (LangSmith vs. open-source vs. custom) that parallel the orchestration build-vs-buy decision.
- **[GenAI Design Patterns](../12-patterns/01-genai-design-patterns.md)**: Broader architectural patterns that inform framework selection.
