...
**Cross-references:** [Transformers](../01-foundations/transformers.md) | [Model Serving](model-serving.md) | [GPU Compute](gpu-compute.md) | [Context Scaling](context-scaling.md) | [Latency Optimization](../11-performance/latency-optimization.md)
...
- **[Transformers](../01-foundations/transformers.md):** The attention mechanism's Q/K/V structure creates the KV cache. Attention variant (MHA/GQA/MQA) determines cache size per request
- **[Model Serving](model-serving.md):** The serving engine manages KV cache lifecycle through PagedAttention, continuous batching, and prefix caching
- **[GPU Compute](gpu-compute.md):** GPU HBM capacity and memory bandwidth are the physical constraints on KV cache size and access speed. NVLink bandwidth determines KV cache transfer speed in tensor-parallel and disaggregated setups
- **[Context Scaling](context-scaling.md):** Long-context models (128K+) stress KV cache management to its limits. Techniques like sliding window, attention sinks, and context compression directly interact with KV cache sizing
- **[Latency Optimization](../11-performance/latency-optimization.md):** TTFT is determined by prefill time (compute-bound). Inter-token latency is determined by decode time (memory-bandwidth-bound due to KV cache reads). Both are directly influenced by KV cache management decisions
- **[Quantization](quantization.md):** KV cache can be independently quantized (FP8/INT8) to double effective batch size. This is orthogonal to and composable with weight quantization
