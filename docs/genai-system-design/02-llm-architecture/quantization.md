...
**Cross-references:** [Model Serving](model-serving.md) | [GPU Compute](gpu-compute.md) | [Fine-Tuning](../03-model-strategies/fine-tuning.md) | [Cost Optimization](../11-performance/cost-optimization.md)
...
- **[Model Serving](model-serving.md):** Quantized models are consumed by serving engines (vLLM, TensorRT-LLM, llama.cpp) that must support the specific quantized format. The serving engine's kernel library determines actual throughput
- **[GPU Compute](gpu-compute.md):** Hardware determines which quantization formats are accelerated (INT8 Tensor Cores on A100, FP8 Tensor Cores on H100, NEON/AVX on CPU)
- **[Fine-Tuning](../03-model-strategies/fine-tuning.md):** QLoRA uses NF4 quantization to enable parameter-efficient fine-tuning on limited hardware. QAT integrates quantization into the training process
- **[Cost Optimization](../11-performance/cost-optimization.md):** Quantization is the single largest lever for reducing inference cost — 2–4x GPU memory reduction translates directly to 2–4x cost reduction per token
- **[KV Cache Management](kv-cache.md):** KV cache quantization (FP8/INT8) is an independent optimization that compounds with weight quantization for maximum memory efficiency
- **[Transformers](../01-foundations/transformers.md):** Understanding attention head configuration (MHA vs. GQA vs. MQA) determines which weight tensors are quantized and their relative importance
