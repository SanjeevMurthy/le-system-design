...
- [Context Management](../06-prompt-engineering/03-context-management.md) --- Token budgeting strategies, context window optimization, and prompt assembly patterns used in conversation history management.
- [Guardrails](../10-safety/01-guardrails.md) --- Deep dive into moderation frameworks (Guardrails AI, NeMo Guardrails, Llama Guard) that implement the chatbot's safety pipeline.
- [Latency Optimization](../11-performance/01-latency-optimization.md) --- End-to-end latency analysis, TTFT optimization, and streaming delivery patterns.
- [Model Routing](../11-performance/04-model-routing.md) --- Cost/latency-based model selection logic used by the chatbot's model router.
- [Model Serving](../02-llm-architecture/01-model-serving.md) --- LLM inference infrastructure (vLLM, TGI, TensorRT-LLM) that backs the chatbot's generation layer.
- [RAG Pipeline](../04-rag/01-rag-pipeline.md) --- Knowledge-grounded chatbots use RAG to retrieve context from external knowledge bases.
- [PII Protection](../10-safety/02-pii-protection.md) --- PII detection and redaction in both input and output moderation stages.
- [Prompt Injection](../06-prompt-engineering/04-prompt-injection.md) --- Injection detection as a critical input moderation component.
