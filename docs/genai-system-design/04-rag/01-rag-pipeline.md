...
- **Document ingestion pipeline** (upstream): Converts raw documents into chunked, embedded, indexed content. Changes here propagate to retrieval quality. See [Document Ingestion](./02-document-ingestion.md).
- **Chunking layer** (upstream): Determines the granularity of retrieval units. See [Chunking Strategies](./03-chunking.md).
- **Vector database** (infrastructure): Stores and serves embeddings for ANN search. See [Vector Databases](../05-vector-search/02-vector-databases.md).
- **Embedding models** (infrastructure): Produce the vector representations. See [Embeddings](../01-foundations/04-embeddings.md).
- **Reranking models** (inline): Cross-encoders that re-score retrieved candidates. See [Retrieval & Reranking](./04-retrieval-reranking.md).
- **Evaluation framework** (feedback loop): Measures faithfulness, relevance, and hallucination rates. See [Eval Frameworks](../09-evaluation/01-eval-frameworks.md).
...
- **[Document Ingestion](./02-document-ingestion.md)**: How raw documents are parsed, cleaned, and prepared for the RAG pipeline.
- **[Chunking Strategies](./03-chunking.md)**: How documents are split into retrieval units --- directly determines retrieval quality.
- **[Retrieval & Reranking](./04-retrieval-reranking.md)**: Deep dive into retrieval algorithms, reranker architectures, and hybrid search.
- **[Vector Databases](../05-vector-search/02-vector-databases.md)**: Infrastructure for storing and querying embeddings at scale.
- **[Embeddings](../01-foundations/04-embeddings.md)**: The vector representations that power dense retrieval.
- **[Eval Frameworks](../09-evaluation/01-eval-frameworks.md)**: How to measure and monitor RAG quality in production.
- **[Context Scaling](../02-llm-architecture/05-context-scaling.md)**: Long-context models as an alternative or complement to RAG.
- **[Multimodal RAG](./06-multimodal-rag.md)**: Extending RAG to images, tables, and other non-text modalities.
- **[Enterprise Search](../13-case-studies/03-enterprise-search.md)**: Full-stack enterprise search systems that build on RAG.
