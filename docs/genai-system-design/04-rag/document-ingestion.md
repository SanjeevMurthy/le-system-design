...
- **Vector database** (downstream): Receives embedded chunks and metadata. See [Vector Databases](../05-vector-search/vector-databases.md).
...
- **[Embeddings](../01-foundations/embeddings.md)**: The embedding models used in the final ingestion stage.
- **[Vector Databases](../05-vector-search/vector-databases.md)**: The storage layer where embedded chunks are indexed.
- **[Multimodal RAG](./multimodal-rag.md)**: Extends ingestion to handle images, audio, and video alongside text.
- **[Enterprise Search](../13-case-studies/enterprise-search.md)**: Full-stack enterprise search systems that depend on robust ingestion.
