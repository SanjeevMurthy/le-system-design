...
- **Multimodal models** ([05-multimodal-models.md](../01-foundations/05-multimodal-models.md)): The generation model must be vision-language capable (GPT-4o, Gemini 2.0, Claude 3.5 Sonnet, Llama 3.2 Vision) to consume image and table context.
- **Document ingestion** ([02-document-ingestion.md](./02-document-ingestion.md)): The parsing pipeline that extracts text, images, and tables from source documents feeds directly into multimodal RAG's embedding stage.
- **RAG pipeline** ([01-rag-pipeline.md](./01-rag-pipeline.md)): Multimodal RAG extends the standard RAG pipeline with modality-specific embedding, indexing, and context assembly.
- **Embedding models** ([04-embeddings.md](../01-foundations/04-embeddings.md)): Text embedding models handle the textual modality; vision embedding models (CLIP, SigLIP) handle images.
- **Vector databases** ([02-vector-databases.md](../05-vector-search/02-vector-databases.md)): Must support multiple indices (one per modality) or multi-vector storage.
...
- [Multimodal Models](../01-foundations/05-multimodal-models.md) --- Architecture and capabilities of vision-language models used for multimodal generation and understanding.
- [Document Ingestion](./02-document-ingestion.md) --- Parsing, extraction, and preprocessing pipelines that feed multimodal RAG.
- [RAG Pipeline](./01-rag-pipeline.md) --- The end-to-end RAG orchestration that multimodal RAG extends.
- [Embedding Models](../01-foundations/04-embeddings.md) --- Text embedding models and their role alongside vision embeddings.
- [Vector Databases](../05-vector-search/02-vector-databases.md) --- Storage and retrieval infrastructure for multimodal embeddings, including multi-vector support.
- [Retrieval and Reranking](./04-retrieval-reranking.md) --- Dense/sparse/hybrid retrieval and cross-encoder reranking applied to multimodal contexts.
- [GraphRAG](./05-graphrag.md) --- Combining knowledge graphs with multimodal retrieval for entity-rich visual documents.
- [ColBERT and Late Interaction](./04-retrieval-reranking.md#35-colbert-late-interaction) --- The late interaction scoring mechanism used by ColPali.
