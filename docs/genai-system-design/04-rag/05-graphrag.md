...
- **RAG pipeline** ([01-rag-pipeline.md](./01-rag-pipeline.md)): GraphRAG extends the standard RAG pipeline with an additional graph-based retrieval path. The overall orchestration (prompt construction, generation, citation) remains the same.
- **Retrieval and reranking** ([04-retrieval-reranking.md](./04-retrieval-reranking.md)): Graph-retrieved context and vector-retrieved context are merged and optionally reranked before entering the LLM prompt. Cross-encoder rerankers can score graph-derived text passages alongside vector-retrieved passages.
- **Enterprise search** ([03-enterprise-search.md](../13-case-studies/03-enterprise-search.md)): Enterprise knowledge bases with structured relationships (org charts, product hierarchies, supply chains) benefit heavily from GraphRAG.
- **Agent architectures** ([01-agent-architecture.md](../07-agents/01-agent-architecture.md)): Agents can use graph queries as a tool --- "find all entities related to X within 2 hops" --- enabling structured reasoning that vector search alone cannot provide.
...
- [RAG Pipeline](./01-rag-pipeline.md) --- The broader RAG architecture within which GraphRAG operates.
- [Retrieval and Reranking](./04-retrieval-reranking.md) --- Dense/sparse retrieval and cross-encoder reranking that complement graph-based retrieval.
- [Enterprise Search](../13-case-studies/03-enterprise-search.md) --- Enterprise knowledge management use cases where GraphRAG adds the most value.
- [Agent Architecture](../07-agents/01-agent-architecture.md) --- Agents that use graph queries as tools for structured reasoning.
- [Embedding Models](../01-foundations/04-embeddings.md) --- Embeddings used for entity description indexing and vector-based graph search.
- [Multimodal RAG](./06-multimodal-rag.md) --- Extending GraphRAG to non-text modalities (visual entities, audio entities).
- [Evaluation Frameworks](../09-evaluation/01-eval-frameworks.md) --- Measuring GraphRAG quality, including multi-hop accuracy and answer comprehensiveness.
