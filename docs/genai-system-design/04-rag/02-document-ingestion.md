...
- **Chunking layer** (downstream): Ingestion produces clean text; chunking splits it into retrieval units. In practice, chunking is often a stage within the ingestion pipeline. See [Chunking Strategies](./03-chunking.md).
- **RAG pipeline** (downstream consumer): The quality of retrieval depends entirely on what was ingested. See [RAG Pipeline](./01-rag-pipeline.md).
- **Vector database** (downstream): Receives embedded chunks and metadata. See [Vector Databases](../05-vector-search/02-vector-databases.md).
...
- **[RAG Pipeline Architecture](./01-rag-pipeline.md)**: The downstream consumer of ingested content. Ingestion quality directly bounds retrieval quality.
- **[Chunking Strategies](./03-chunking.md)**: The next stage after parsing. How documents are split into retrieval units.
- **[Embeddings](../01-foundations/04-embeddings.md)**: The embedding models used in the final ingestion stage.
- **[Vector Databases](../05-vector-search/02-vector-databases.md)**: The storage layer where embedded chunks are indexed.
- **[Multimodal RAG](./06-multimodal-rag.md)**: Extends ingestion to handle images, audio, and video alongside text.
- **[Enterprise Search](../13-case-studies/03-enterprise-search.md)**: Full-stack enterprise search systems that depend on robust ingestion.
