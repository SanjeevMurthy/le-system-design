...
- **Vector databases** ([02-vector-databases.md](./02-vector-databases.md)): Some databases (Weaviate, Qdrant, Milvus, Pinecone) support hybrid search natively --- both dense and sparse retrieval in a single query. Others require an external sparse retrieval system.
- **Embedding models** ([01-embedding-models.md](./01-embedding-models.md)): Dense retrieval quality is bounded by the embedding model. Some models (BGE-M3) produce both dense and sparse representations, enabling single-model hybrid search.
- **ANN algorithms** ([03-ann-algorithms.md](./03-ann-algorithms.md)): Dense retrieval uses ANN search. The ANN algorithm interacts with metadata filtering, which is related to but distinct from hybrid search fusion.
- **Retrieval and reranking** ([../04-rag/04-retrieval-reranking.md](../04-rag/04-retrieval-reranking.md)): Hybrid search produces the candidate set for reranking. Better hybrid candidates lead to better reranked results.
- **RAG pipeline** ([../04-rag/01-rag-pipeline.md](../04-rag/01-rag-pipeline.md)): Hybrid search is a component within the broader RAG orchestration.
...
- [Retrieval and Reranking](../04-rag/04-retrieval-reranking.md) --- the two-stage pattern where hybrid search produces the candidate set
- [Vector Databases](./02-vector-databases.md) --- infrastructure that implements hybrid search natively or serves the dense component
- [Embedding Models](./01-embedding-models.md) --- models that produce dense vectors, and in some cases (BGE-M3) sparse vectors
- [ANN Algorithms](./03-ann-algorithms.md) --- algorithms powering the dense retrieval component
- [RAG Pipeline](../04-rag/01-rag-pipeline.md) --- the end-to-end architecture where hybrid search is a retrieval strategy
- [Embeddings](../01-foundations/04-embeddings.md) --- foundational concepts of dense vector representations
