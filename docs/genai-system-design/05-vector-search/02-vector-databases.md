...
- **Embedding models** ([01-embedding-models.md](./01-embedding-models.md)): The dimensionality, data type, and distance metric of the embeddings determine index configuration. Switching embedding models requires re-indexing the entire corpus.
- **ANN algorithms** ([03-ann-algorithms.md](./03-ann-algorithms.md)): The index type (HNSW, IVF, DiskANN) is a configuration choice within the vector database. Not all databases support all index types.
- **Hybrid search** ([04-hybrid-search.md](./04-hybrid-search.md)): Some vector databases natively support sparse vectors and BM25 alongside dense search. Others require external keyword search systems.
- **RAG pipeline** ([../04-rag/01-rag-pipeline.md](../04-rag/01-rag-pipeline.md)): The vector database is the primary retrieval backend for RAG. Query latency, filtering capability, and result metadata directly affect RAG quality and speed.
- **Retrieval and reranking** ([../04-rag/04-retrieval-reranking.md](../04-rag/04-retrieval-reranking.md)): The vector database produces the initial candidate set that the reranker refines.
...
- [ANN Algorithms](./03-ann-algorithms.md) --- the index algorithms that vector databases implement internally
- [Embedding Models](./01-embedding-models.md) --- the models that produce the vectors stored in these databases
- [Hybrid Search](./04-hybrid-search.md) --- combining dense and sparse retrieval, often within the vector database
- [RAG Pipeline](../04-rag/01-rag-pipeline.md) --- the end-to-end retrieval-augmented generation architecture
- [Retrieval and Reranking](../04-rag/04-retrieval-reranking.md) --- the two-stage search pattern that consumes vector database results
- [Embeddings](../01-foundations/04-embeddings.md) --- foundational concepts of vector representations
