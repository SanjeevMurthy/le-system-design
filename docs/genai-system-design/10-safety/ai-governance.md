...
- **Red teaming**: Provides the empirical evidence that governance decisions are based on --- deployment approvals require red team results demonstrating acceptable risk levels. See [red-teaming.md](red-teaming.md).
- **Guardrails**: Governance policies define what guardrails must enforce. The guardrail configuration is a technical implementation of governance requirements. See [guardrails.md](guardrails.md).
- **PII protection**: Data privacy governance (GDPR Article 22, CCPA) directly constrains how personal data flows through GenAI pipelines. See [pii-protection.md](pii-protection.md).
- **Evaluation frameworks**: Governance mandates which evaluations must pass before deployment. Eval results are the primary evidence artifact for audit trails. See [eval-frameworks.md](../09-evaluation/eval-frameworks.md).
- **Observability**: Governance requires logging, monitoring, and audit trails that the observability infrastructure must provide. See [llm-observability.md](../09-evaluation/llm-observability.md).
...
- [red-teaming.md](red-teaming.md) --- Adversarial testing that provides the empirical evidence for governance decisions
- [guardrails.md](guardrails.md) --- Runtime policy enforcement that implements governance requirements
- [pii-protection.md](pii-protection.md) --- Data privacy governance for GenAI systems
- [eval-frameworks.md](../09-evaluation/eval-frameworks.md) --- Evaluation infrastructure that produces governance evidence artifacts
- [llm-observability.md](../09-evaluation/llm-observability.md) --- Monitoring infrastructure that enables continuous compliance
- [alignment.md](../01-foundations/alignment.md) --- Training-time safety that governance validates and relies upon
