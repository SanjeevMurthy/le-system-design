...
- **Alignment validation**: Red teaming directly tests whether RLHF/DPO training holds under adversarial pressure. Findings feed back into safety tuning data. See [06-alignment.md](../01-foundations/06-alignment.md).
- **Guardrail testing**: Every guardrail rule and classifier must be tested against the current attack taxonomy. Red teaming discovers the gaps between policy intent and enforcement reality. See [01-guardrails.md](01-guardrails.md).
- **Prompt injection overlap**: Many red team attacks are prompt injections with a safety-harm payload. The two disciplines share attack surface analysis but differ in goals --- prompt injection focuses on control-flow hijacking; red teaming focuses on harm elicitation. See [04-prompt-injection.md](../06-prompt-engineering/04-prompt-injection.md).
- **Evaluation frameworks**: Red team findings become evaluation datasets. Attack-response pairs, labeled by severity and harm category, form the basis of safety benchmarks. See [01-eval-frameworks.md](../09-evaluation/01-eval-frameworks.md).
- **Governance**: Red teaming provides the evidence base for governance decisions --- deployment approvals, risk assessments, regulatory compliance documentation. See [04-ai-governance.md](04-ai-governance.md).
...
These combine prompt injection techniques with harm-elicitation goals. See [04-prompt-injection.md](../06-prompt-engineering/04-prompt-injection.md) for the injection techniques themselves. In a red teaming context, the focus is on the harmful payload rather than the injection mechanism.
...
- [04-prompt-injection.md](../06-prompt-engineering/04-prompt-injection.md) --- Injection techniques that form the attack substrate for many red team strategies
- [01-guardrails.md](01-guardrails.md) --- The defense mechanisms that red teaming validates and stress-tests
- [06-alignment.md](../01-foundations/06-alignment.md) --- The training-time safety that red teaming evaluates for robustness
- [01-eval-frameworks.md](../09-evaluation/01-eval-frameworks.md) --- Red team findings become safety evaluation benchmarks
- [04-ai-governance.md](04-ai-governance.md) --- Red teaming provides the evidence base for governance decisions and regulatory compliance
- [04-llm-observability.md](../09-evaluation/04-llm-observability.md) --- Production monitoring that detects attacks red teaming missed
