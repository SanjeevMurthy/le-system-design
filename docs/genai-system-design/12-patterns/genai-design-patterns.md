...
- **[Guardrails](../10-safety/guardrails.md)** (peer): The Guard Rails Pattern defined here is an architectural abstraction; the guardrails topic covers implementation frameworks (Guardrails AI, NeMo Guardrails, Llama Guard) in depth.
- **[RAG Pipeline](../04-rag/rag-pipeline.md)** (peer): The RAG Pattern here defines the canonical three-phase flow; the RAG pipeline topic covers chunking, retrieval, reranking, and evaluation.
- **[Agent Architecture](../07-agents/agent-architecture.md)** (peer): The Agent Loop Pattern here defines the Plan-Act-Observe-Reflect cycle; the agent architecture topic covers ReAct, Plan-and-Execute, LATS, and Reflexion variants.
- **[Circuit Breaker](../../traditional-system-design/08-resilience/circuit-breaker.md)** (foundation): The traditional circuit breaker pattern. The GenAI variant adds LLM-specific health signals (quality degradation, not just errors).
- **[Rate Limiting](../../traditional-system-design/08-resilience/rate-limiting.md)** (foundation): Provider rate limits are the primary constraint that the Gateway and Bulkhead patterns manage.
- **[Model Selection](../03-model-strategies/model-selection.md)** (upstream): Model routing decisions feed into the Gateway, Cascade, and Shadow Mode patterns.
- **[LLM Observability](../09-evaluation/llm-observability.md)** (cross-cutting): Every pattern produces telemetry that the observability layer must capture.
...
**The GenAI-specific extension to the traditional circuit breaker** (see [Circuit Breaker](../../traditional-system-design/08-resilience/circuit-breaker.md) for the general pattern): LLM circuit breakers must monitor quality signals in addition to availability signals. A provider can return HTTP 200 with garbage output --- this is not a traditional error, but it is a failure. Quality-aware circuit breakers track:
...
For deep coverage of implementation, see [Semantic Caching](../11-performance/semantic-caching.md).
...
For deep coverage of each phase, see [RAG Pipeline](../04-rag/rag-pipeline.md), [Chunking](../04-rag/chunking.md), [Retrieval & Reranking](../04-rag/retrieval-reranking.md), and [Document Ingestion](../04-rag/document-ingestion.md).
...
For deep coverage, see [Agent Architecture](../07-agents/agent-architecture.md), [Tool Use](../07-agents/tool-use.md), and [Multi-Agent Systems](../07-agents/multi-agent.md).
...
For deep coverage of guardrail frameworks and implementation, see [Guardrails](../10-safety/guardrails.md), [Prompt Injection](../06-prompt-engineering/prompt-injection.md), and [PII Protection](../10-safety/pii-protection.md).
...
For related coverage, see [Model Selection](../03-model-strategies/model-selection.md).
...
For prompt design patterns, see [Prompt Patterns](../06-prompt-engineering/prompt-patterns.md) and [Context Management](../06-prompt-engineering/context-management.md).
...
- **[Model Routing](../11-performance/model-routing.md)**: Dynamic model selection based on query characteristics. Extends the Gateway and Cascade patterns with ML-based routing decisions.
- **[Guardrails](../10-safety/guardrails.md)**: Deep coverage of guardrail frameworks (Guardrails AI, NeMo Guardrails, Llama Guard), implementation code, and framework comparison.
- **[Semantic Caching](../11-performance/semantic-caching.md)**: Detailed treatment of embedding-based cache architecture, similarity thresholds, invalidation strategies, and vector store backends.
- **[RAG Pipeline](../04-rag/rag-pipeline.md)**: Full coverage of retrieval, chunking, reranking, and evaluation for the RAG pattern.
- **[Agent Architecture](../07-agents/agent-architecture.md)**: ReAct, Plan-and-Execute, LATS, Reflexion, and other agent loop variants with implementation depth.
...
- **[Circuit Breaker](../../traditional-system-design/08-resilience/circuit-breaker.md)**: The foundational circuit breaker pattern from distributed systems, extended here for LLM-specific health signals.
- **[Rate Limiting](../../traditional-system-design/08-resilience/rate-limiting.md)**: Token bucket, sliding window, and other rate limiting algorithms that underpin the Gateway and Bulkhead patterns.
- **[Eval Frameworks](../09-evaluation/eval-frameworks.md)**: Evaluation frameworks that power the confidence gates, shadow mode comparisons, and guard rail accuracy measurement.
- **[LLM Observability](../09-evaluation/llm-observability.md)**: Tracing and metrics infrastructure that every pattern in this catalog depends on for operational visibility.
