...
- **[Model Routing and Cascading](model-routing.md):** The primary mechanism for reducing cost through intelligent model selection per query
- **[Semantic Caching](semantic-caching.md):** Caching strategies that eliminate LLM inference entirely for repeated or similar queries
- **[Latency Optimization](latency-optimization.md):** Cost and latency optimizations are deeply intertwined -- faster inference means more throughput per GPU dollar
- **[Model Selection Criteria](../03-model-strategies/model-selection.md):** How to evaluate models on the cost-quality-latency frontier
- **[Quantization](../02-llm-architecture/quantization.md):** FP8 and INT4 quantization that doubles or quadruples tokens-per-GPU, directly reducing serving cost
- **[Fine-Tuning](../03-model-strategies/fine-tuning.md):** Fine-tuning a small model to match a large model's quality on specific tasks is a powerful cost optimization
- **[Distillation](../03-model-strategies/distillation.md):** Training a smaller student model to replicate a larger teacher model's behavior at a fraction of the inference cost
- **[Model Serving Infrastructure](../02-llm-architecture/model-serving.md):** Serving framework choices that affect throughput and thus per-token cost
- **[Rate Limiting](../../traditional-system-design/08-resilience/rate-limiting.md):** Rate limiting as a cost control mechanism to prevent budget overruns
- **[Context Window Management](../06-prompt-engineering/context-management.md):** Context optimization directly reduces input token cost
