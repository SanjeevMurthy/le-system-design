...
- **[Model Serving Infrastructure](../02-llm-architecture/01-model-serving.md):** Serving frameworks (vLLM, TensorRT-LLM, TGI, SGLang) that implement the latency optimization techniques discussed here
- **[KV Cache Management](../02-llm-architecture/04-kv-cache.md):** PagedAttention, prefix caching, KV cache quantization -- the memory management layer that directly determines TTFT and maximum concurrency
- **[Semantic Caching](02-semantic-caching.md):** Cache-based latency optimization that eliminates LLM inference entirely for previously seen queries
- **[Model Routing and Cascading](04-model-routing.md):** Routing simple queries to fast models as a latency optimization strategy
- **[Cost Optimization](03-cost-optimization.md):** Cost-latency tradeoffs including batch inference, model selection, and quantization decisions
- **[Context Window Management](../06-prompt-engineering/03-context-management.md):** Prompt compression, context assembly, and streaming strategies that directly affect prefill latency
- **[Quantization](../02-llm-architecture/03-quantization.md):** FP8, INT4, and other quantization methods that reduce memory reads and accelerate both prefill and decode
- **[GPU Compute](../02-llm-architecture/02-gpu-compute.md):** Hardware specifications (TFLOPS, memory bandwidth, HBM capacity) that set the physical limits of latency optimization
- **[Load Balancing](../../traditional-system-design/02-scalability/01-load-balancing.md):** Request distribution strategies including prefix-aware routing and least-connections balancing
