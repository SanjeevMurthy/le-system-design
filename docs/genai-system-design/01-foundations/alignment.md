...
- **Model selection**: The alignment pipeline determines which model variants are available (base, instruct, chat) and their behavioral profiles. See [llm-landscape.md](llm-landscape.md).
- **Fine-tuning**: Custom fine-tuning (LoRA, QLoRA) inserts into the post-training pipeline. Poorly designed fine-tuning data can undo safety alignment. See [fine-tuning.md](../03-model-strategies/fine-tuning.md).
- **Guardrails**: Runtime guardrails are the final defense layer after alignment. They compensate for alignment failures and enforce application-specific policies. See [guardrails.md](../10-safety/guardrails.md).
- **Red teaming**: Adversarial testing validates alignment robustness. See [red-teaming.md](../10-safety/red-teaming.md).
...
**System design implication**: No alignment technique eliminates all adversarial attacks. Defense-in-depth requires alignment training + runtime guardrails + monitoring + incident response. See [guardrails.md](../10-safety/guardrails.md) and [red-teaming.md](../10-safety/red-teaming.md).
...
- [llm-landscape.md](llm-landscape.md) -- Overview of LLM families (GPT, Claude, Gemini, LLaMA, Mistral) and their alignment approaches.
- [fine-tuning.md](../03-model-strategies/fine-tuning.md) -- LoRA, QLoRA, full fine-tuning: the techniques used to implement SFT and DPO in practice. Covers the risk of alignment erasure.
- [guardrails.md](../10-safety/guardrails.md) -- Runtime safety filters (Guardrails AI, NeMo Guardrails, Llama Guard) that compensate for alignment limitations.
- [red-teaming.md](../10-safety/red-teaming.md) -- Adversarial testing methodologies for validating alignment robustness.
- [transformers.md](transformers.md) -- The transformer architecture that alignment techniques optimize on top of.
- [prompt-injection.md](../06-prompt-engineering/prompt-injection.md) -- Adversarial prompt attacks that alignment must defend against.
- [benchmarks.md](../09-evaluation/benchmarks.md) -- MT-Bench, AlpacaEval, Chatbot Arena, and other alignment quality benchmarks.
- [eval-frameworks.md](../09-evaluation/eval-frameworks.md) -- Evaluation tools (RAGAS, DeepEval) for measuring alignment-relevant properties like helpfulness and harmlessness.
- [cost-optimization.md](../11-performance/cost-optimization.md) -- The cost implications of extended thinking and alignment tax.
- [ai-governance.md](../10-safety/ai-governance.md) -- Regulatory frameworks (EU AI Act, NIST AI RMF) that mandate alignment properties.
