...
- **[Transformer Architecture](01-transformers.md):** The foundational architecture underlying every model discussed here — understanding attention variants, positional encoding, and MoE is prerequisite knowledge
- **[Model Selection Framework](../03-model-strategies/01-model-selection.md):** Detailed decision framework for choosing models based on task requirements, cost constraints, and organizational context
- **[Model Serving & Inference](../02-llm-architecture/01-model-serving.md):** How to deploy and serve the models described here — vLLM, TGI, TensorRT-LLM configurations for each model family
- **[Fine-Tuning Strategies](../03-model-strategies/02-fine-tuning.md):** When and how to fine-tune open-weight models from this landscape (LoRA, QLoRA, full fine-tuning)
- **[Quantization](../02-llm-architecture/03-quantization.md):** Quantization techniques and their impact on the models listed here — which models quantize well and which are sensitive
- **[Cost Optimization](../11-performance/03-cost-optimization.md):** Deep dive into the cost modeling framework introduced in Section 6.3
