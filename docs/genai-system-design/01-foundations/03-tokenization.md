...
**GenAI foundations**:
- [Transformers](./01-transformers.md) --- tokenization feeds directly into the embedding layer of transformer models
- [LLM Landscape](./02-llm-landscape.md) --- each model family uses a specific tokenizer; vocabulary size is a key differentiator
- [Embeddings](./04-embeddings.md) --- token IDs are the input to the embedding lookup table

**Context and cost**:
- [Context Management](../06-prompt-engineering/03-context-management.md) --- token budgeting, truncation, and prompt assembly depend on tokenizer behavior
- [Cost Optimization](../11-performance/03-cost-optimization.md) --- token pricing is the primary cost driver; tokenizer efficiency directly affects spend
- [KV Cache](../02-llm-architecture/04-kv-cache.md) --- cache size scales with sequence length measured in tokens

**Retrieval and chunking**:
- [Chunking](../04-rag/03-chunking.md) --- token-aware chunking requires the tokenizer to determine split points
- [RAG Pipeline](../04-rag/01-rag-pipeline.md) --- the tokenizer governs context budget allocation for retrieved passages

**Safety and quality**:
- [Prompt Injection](../06-prompt-engineering/04-prompt-injection.md) --- tokenization-level attacks exploit subword boundaries
- [Guardrails](../10-safety/01-guardrails.md) --- safety filters operate at both token and text levels

**Traditional system design**:
- [Search and Indexing](../../traditional-system-design/11-patterns/05-search-and-indexing.md) --- tokenization in NLP search vs tokenization in LLMs share conceptual roots but differ in implementation
