...
- **Tokenization** (foundation): Token counting is the metrological basis of context management. See [Tokenization](../01-foundations/tokenization.md).
- **Prompt patterns** (peer): Few-shot examples, chain-of-thought, and other patterns consume context budget. See [Prompt Patterns](./prompt-patterns.md).
- **RAG pipeline** (upstream): RAG determines how much retrieved context competes for the context budget. See [RAG Pipeline](../04-rag/rag-pipeline.md).
- **Context scaling** (infrastructure): Long-context model architectures expand the budget but introduce new tradeoffs. See [Context Scaling](../02-llm-architecture/context-scaling.md).
- **Cost optimization** (downstream): Token usage is the primary cost driver. Context management directly controls cost. See [Cost Optimization](../11-performance/cost-optimization.md).
- **Semantic caching** (optimization): Caching frequent prompt prefixes reduces redundant token processing. See [Semantic Caching](../11-performance/semantic-caching.md).
- **KV cache** (infrastructure): The context window's runtime representation. Understanding KV cache helps explain why context length affects latency. See [KV Cache](../02-llm-architecture/kv-cache.md).
...
- **[Tokenization](../01-foundations/tokenization.md)**: The foundation of token counting --- understanding BPE, SentencePiece, and vocabulary design.
- **[Prompt Patterns](./prompt-patterns.md)**: Few-shot, chain-of-thought, and other patterns that consume context budget and must be managed.
- **[RAG Pipeline](../04-rag/rag-pipeline.md)**: Retrieved context is typically the largest variable consumer of the context budget.
- **[Context Scaling](../02-llm-architecture/context-scaling.md)**: Long-context architectures (RoPE scaling, ring attention) that expand the budget.
- **[Cost Optimization](../11-performance/cost-optimization.md)**: Context management is the primary lever for controlling inference cost.
- **[Semantic Caching](../11-performance/semantic-caching.md)**: Caching entire responses for semantically similar queries, complementing prompt-level caching.
- **[KV Cache](../02-llm-architecture/kv-cache.md)**: The runtime data structure that represents context --- understanding it explains why context length affects latency and memory.
- **[Model Serving](../02-llm-architecture/model-serving.md)**: Serving infrastructure that implements prompt caching and manages KV cache memory.
