...
- [API Gateway](../architecture/api-gateway.md) -- the most common placement point for rate limiting
- [Circuit Breaker](./circuit-breaker.md) -- exponential backoff and jitter for retry storms (canonical detail there)
- [Redis](../caching/redis.md) -- the standard backing store for rate limit state (atomic INCR, Lua scripts)
- [Feature Flags](./feature-flags.md) -- can be used to dynamically adjust rate limits during incidents
- [Load Balancing](../scalability/load-balancing.md) -- load balancers can enforce basic rate limits at L7
