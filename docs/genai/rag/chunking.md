...
- **Embedding model** (downstream): The chunk must fit within the embedding model's context window. Chunk size relative to model capacity determines embedding quality. See [Embeddings](../foundations/embeddings.md).
...
- **Context management** (downstream): The number and size of chunks that fit in the LLM context window depend on chunk size decisions made here. See [Context Management](../orchestration/context-management.md).
...
- **[Embeddings](../foundations/embeddings.md)**: Chunk size must align with the embedding model's context window and strengths.
...
- **[Context Management](../orchestration/context-management.md)**: Chunk size and count determine how much context the LLM receives and at what cost.
- **[Vector Databases](../vector-search/vector-databases.md)**: Chunk count determines vector count, which determines storage cost and search latency.
