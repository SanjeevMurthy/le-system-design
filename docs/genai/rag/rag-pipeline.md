...
- **Vector database** (infrastructure): Stores and serves embeddings for ANN search. See [Vector Databases](../vector-search/vector-databases.md).
- **Embedding models** (infrastructure): Produce the vector representations. See [Embeddings](../foundations/embeddings.md).
- **Reranking models** (inline): Cross-encoders that re-score retrieved candidates. See [Retrieval & Reranking](./retrieval-reranking.md).
- **Evaluation framework** (feedback loop): Measures faithfulness, relevance, and hallucination rates. See [Eval Frameworks](../evaluation/eval-frameworks.md).
...
- **[Vector Databases](../vector-search/vector-databases.md)**: Infrastructure for storing and querying embeddings at scale.
- **[Embeddings](../foundations/embeddings.md)**: The vector representations that power dense retrieval.
- **[Eval Frameworks](../evaluation/eval-frameworks.md)**: How to measure and monitor RAG quality in production.
- **[Context Scaling](../llm-architecture/context-scaling.md)**: Long-context models as an alternative or complement to RAG.
- **[Multimodal RAG](./multimodal-rag.md)**: Extending RAG to images, tables, and other non-text modalities.
- **[Enterprise Search](./enterprise-search.md)**: Full-stack enterprise search systems that build on RAG.
