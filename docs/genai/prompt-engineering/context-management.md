...
- **Tokenization** (foundation): Token counting is the metrological basis of context management. See [Tokenization](../foundations/tokenization.md).
...
- **RAG pipeline** (upstream): RAG determines how much retrieved context competes for the context budget. See [RAG Pipeline](../rag/rag-pipeline.md).
- **Context scaling** (infrastructure): Long-context model architectures expand the budget but introduce new tradeoffs. See [Context Scaling](../llm-architecture/context-scaling.md).
- **Cost optimization** (downstream): Token usage is the primary cost driver. Context management directly controls cost. See [Cost Optimization](../performance/cost-optimization.md).
- **Semantic caching** (optimization): Caching frequent prompt prefixes reduces redundant token processing. See [Semantic Caching](../performance/semantic-caching.md).
- **KV cache** (infrastructure): The context window's runtime representation. Understanding KV cache helps explain why context length affects latency. See [KV Cache](../llm-architecture/kv-cache.md).
...
- **[Tokenization](../foundations/tokenization.md)**: The foundation of token counting --- understanding BPE, SentencePiece, and vocabulary design.
...
- **[RAG Pipeline](../rag/rag-pipeline.md)**: Retrieved context is typically the largest variable consumer of the context budget.
- **[Context Scaling](../llm-architecture/context-scaling.md)**: Long-context architectures (RoPE scaling, ring attention) that expand the budget.
- **[Cost Optimization](../performance/cost-optimization.md)**: Context management is the primary lever for controlling inference cost.
- **[Semantic Caching](../performance/semantic-caching.md)**: Caching entire responses for semantically similar queries, complementing prompt-level caching.
- **[KV Cache](../llm-architecture/kv-cache.md)**: The runtime data structure that represents context --- understanding it explains why context length affects latency and memory.
- **[Model Serving](../llm-architecture/model-serving.md)**: Serving infrastructure that implements prompt caching and manages KV cache memory.
