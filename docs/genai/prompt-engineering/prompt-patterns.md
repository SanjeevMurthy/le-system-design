...
- **Agent architecture** (orchestration): Agent loops compose prompts dynamically across multiple turns, layering ReAct, tool-use, and reflection patterns. See [Agent Architecture](../agents/agent-architecture.md).
- **Evaluation frameworks** (feedback loop): Prompt variants are evaluated systematically via evals to select the best-performing pattern for a task. See [Eval Frameworks](../evaluation/eval-frameworks.md).
- **Model serving** (infrastructure): Prompt size and sampling parameters directly impact latency, throughput, and cost at the inference layer. See [Model Serving](../llm-architecture/model-serving.md).
...
- **[Agent Architecture](../agents/agent-architecture.md)**: ReAct, tool-use, and multi-step reasoning patterns form the prompt-level foundation for agent loops.
- **[Eval Frameworks](../evaluation/eval-frameworks.md)**: Prompt variants are compared and optimized using systematic evaluation pipelines.
- **[Fine-Tuning](../model-strategies/fine-tuning.md)**: When prompt engineering reaches its limits, fine-tuning encodes task knowledge into model weights.
- **[Model Serving](../llm-architecture/model-serving.md)**: Prompt size and sampling parameters directly impact serving latency and throughput.
- **[RAG Pipeline](../rag/rag-pipeline.md)**: RAG injects retrieved context into prompts; prompt design determines how effectively the model uses that context.
