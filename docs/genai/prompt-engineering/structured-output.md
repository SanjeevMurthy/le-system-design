...
- **Tool use / function calling** (co-located): Tool use is the primary structured output mechanism for agent systems. See [Tool Use](../agents/tool-use.md).
- **Agent architecture** (consumer): Agents depend on reliable structured output for tool dispatch, state management, and multi-step planning. See [Agent Architecture](../agents/agent-architecture.md).
- **Orchestration frameworks** (infrastructure): LangChain, LlamaIndex, and Semantic Kernel all provide structured output abstractions. See [Orchestration Frameworks](../orchestration/orchestration-frameworks.md).
- **Evaluation frameworks** (quality gate): Output schema conformance is a measurable eval metric. See [Eval Frameworks](../evaluation/eval-frameworks.md).
...
- **[Tool Use](../agents/tool-use.md)**: Function calling / tool use is both a structured output mechanism and an agent capability. The schema definition and validation patterns are shared.
- **[Agent Architecture](../agents/agent-architecture.md)**: Agents depend on reliable structured output for tool dispatch, plan execution, and state management. Schema failures in agent loops cause cascading errors.
- **[Orchestration Frameworks](../orchestration/orchestration-frameworks.md)**: LangChain, LlamaIndex, Semantic Kernel, and Haystack all provide structured output abstractions that wrap the mechanisms described here.
- **[Eval Frameworks](../evaluation/eval-frameworks.md)**: Schema conformance rate is a measurable metric in eval pipelines. Eval datasets with expected schemas test structured output reliability.
- **[Model Serving](../llm-architecture/model-serving.md)**: Constrained decoding requires integration with the serving layer (vLLM, TGI, SGLang). Schema compilation and FSM caching are serving-layer concerns.
