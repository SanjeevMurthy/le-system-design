...
**Cross-references:** [Model Serving](model-serving.md) | [GPU Compute](gpu-compute.md) | [Fine-Tuning](fine-tuning.md) | [Cost Optimization](cost-optimization.md)
...
- **[Fine-Tuning](fine-tuning.md):** QLoRA uses NF4 quantization to enable parameter-efficient fine-tuning on limited hardware. QAT integrates quantization into the training process
- **[Cost Optimization](cost-optimization.md):** Quantization is the single largest lever for reducing inference cost — 2–4x GPU memory reduction translates directly to 2–4x cost reduction per token
...
- **[Transformers](../foundations/transformers.md):** Understanding attention head configuration (MHA vs. GQA vs. MQA) determines which weight tensors are quantized and their relative importance
