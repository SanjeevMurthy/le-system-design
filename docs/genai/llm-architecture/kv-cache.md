...
**Cross-references:** [Transformers](../foundations/transformers.md) | [Model Serving](model-serving.md) | [GPU Compute](gpu-compute.md) | [Context Scaling](context-scaling.md) | [Latency Optimization](latency-optimization.md)
...
- **[Transformers](../foundations/transformers.md):** The attention mechanism's Q/K/V structure creates the KV cache. Attention variant (MHA/GQA/MQA) determines cache size per request
...
- **[Latency Optimization](latency-optimization.md):** TTFT is determined by prefill time (compute-bound). Inter-token latency is determined by decode time (memory-bandwidth-bound due to KV cache reads). Both are directly influenced by KV cache management decisions
