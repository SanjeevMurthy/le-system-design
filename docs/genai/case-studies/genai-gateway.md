...
- [Model Routing](../patterns/model-routing.md) --- Deep dive into routing strategies: weighted, latency-based, cost-based, and capability-based routing logic.
- [Cost Optimization](../patterns/cost-optimization.md) --- Token budgeting, model tiering, prompt compression, and caching strategies for controlling LLM costs.
- [Latency Optimization](../performance/latency-optimization.md) --- End-to-end latency analysis, TTFT optimization, and caching's role in reducing perceived latency.
- [Guardrails](../safety/guardrails.md) --- Input and output moderation frameworks integrated at the gateway layer.
- [Model Serving](../llm-architecture/model-serving.md) --- Self-hosted inference endpoints that the gateway routes to alongside cloud providers.
- [LLM Observability](../evaluation/llm-observability.md) --- Tracing, logging, and monitoring for LLM requests that the gateway feeds into.
- [API Gateway](../../architecture/api-gateway.md) --- Traditional API gateway patterns (Kong, Envoy, AWS API Gateway) that GenAI gateways extend with LLM-specific features.
