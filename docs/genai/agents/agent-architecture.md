...
- **Prompt engineering** (upstream): Agent system prompts define persona, constraints, and output format. See [Prompt Patterns](../prompt-engineering/prompt-patterns.md).
- **Orchestration frameworks** (implementation): LangGraph, CrewAI, AutoGen, and others provide agent runtime primitives. See [Orchestration Frameworks](../orchestration/orchestration-frameworks.md).
- **Structured output** (co-located): Agents depend on reliable structured output for tool dispatch and state serialization. See [Structured Output](../prompt-engineering/structured-output.md).
...
- **[Prompt Patterns](../prompt-engineering/prompt-patterns.md)**: System prompt design for agents, including role definition, constraint specification, and output formatting.
- **[Orchestration Frameworks](../orchestration/orchestration-frameworks.md)**: LangGraph, CrewAI, AutoGen, and other frameworks that implement agent runtime primitives.
- **[Structured Output](../prompt-engineering/structured-output.md)**: Reliable tool call parsing and state serialization that agents depend on.
- **[Context Management](../prompt-engineering/context-management.md)**: Managing the LLM's context window across many agent iterations.
- **[Evaluation Frameworks](../evaluation/eval-frameworks.md)**: Benchmarks and metrics for measuring agent performance (SWE-bench, WebArena, GAIA).
