...
- **[Model Selection Criteria](../model-strategies/model-selection.md):** How to evaluate models on the cost-quality-latency frontier
- **[Quantization](../llm-architecture/quantization.md):** FP8 and INT4 quantization that doubles or quadruples tokens-per-GPU, directly reducing serving cost
- **[Fine-Tuning](../model-strategies/fine-tuning.md):** Fine-tuning a small model to match a large model's quality on specific tasks is a powerful cost optimization
- **[Distillation](../model-strategies/distillation.md):** Training a smaller student model to replicate a larger teacher model's behavior at a fraction of the inference cost
- **[Model Serving Infrastructure](../llm-architecture/model-serving.md):** Serving framework choices that affect throughput and thus per-token cost
- **[Rate Limiting](../../resilience/rate-limiting.md):** Rate limiting as a cost control mechanism to prevent budget overruns
- **[Context Window Management](../prompt-engineering/context-management.md):** Context optimization directly reduces input token cost
