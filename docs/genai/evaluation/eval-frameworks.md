...
- **RAG pipeline** (upstream): Evaluation metrics like faithfulness and context precision directly measure RAG quality. Changes to chunking, retrieval, or reranking are validated through evaluation. See [RAG Pipeline](../rag/rag-pipeline.md).
...
- [RAG Pipeline Architecture](../rag/rag-pipeline.md) --- The primary system being evaluated; evaluation metrics shape RAG design decisions.
- [Guardrails and Safety](../safety/guardrails.md) --- Runtime safety enforcement that evaluation validates.
- [Prompt Patterns](../prompt-engineering/prompt-patterns.md) --- Prompt engineering changes are the most frequent trigger for evaluation.
