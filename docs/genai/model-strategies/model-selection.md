...
**Cross-references:** [LLM Landscape](../foundations/llm-landscape.md) | [Fine-Tuning](fine-tuning.md) | [Model Routing](../orchestration/model-routing.md) | [Cost Optimization](../performance/cost-optimization.md)
...
- **[LLM Landscape](../foundations/llm-landscape.md):** Comprehensive overview of available models, their capabilities, and architectural differences. Foundation for the selection decision.
...
- **[Model Routing](../orchestration/model-routing.md):** Implements the multi-model tiering strategy at runtime, routing requests to the appropriate model based on complexity, cost, and task type.
- **[Cost Optimization](../performance/cost-optimization.md):** Model selection is the single largest lever for cost optimization. A 10x cheaper model that is "good enough" dominates all other cost optimizations.
- **[Model Serving](../llm-architecture/model-serving.md):** Self-hosted model selection determines serving infrastructure requirements.
