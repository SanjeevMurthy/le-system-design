...
2. **API Gateway rate limiting**: Enforces per-client request rate limits. See [Rate Limiting](../resilience/rate-limiting.md) for algorithms (token bucket, sliding window). Rate limits should be keyed to authenticated identity, not just IP, since attackers rotate IPs.
...
5. **Virtual Waiting Room**: For predictable surge events (ticket sales, product launches), a queue-based admission system admits users at a controlled rate. See [Rate Limiting](../resilience/rate-limiting.md) for details.
...
- [Rate Limiting](../resilience/rate-limiting.md) -- algorithms and patterns for request throttling (cross-link only; rate limiting details live there)
- [API Gateway](../architecture/api-gateway.md) -- the centralized enforcement point for API security policies
- [REST API](../api-design/rest-api.md) -- secure design patterns for RESTful APIs
- [Circuit Breaker](../resilience/circuit-breaker.md) -- protects services from cascading failures when security-related checks timeout
